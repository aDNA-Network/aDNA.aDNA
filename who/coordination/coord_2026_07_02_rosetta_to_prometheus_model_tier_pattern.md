...
> **Annotation (2026-07-03, G6 close)**: datapoint **#6** live at `campaign_champollion/artifacts/telemetry_corpus_p6.md` — phase +20% (M6.1 +63%: the per-curation-row pricing calibration for RC/fold-batch missions, ~2.5–3 kT/row; pre-pinning 2-for-2; finding-side-verification rule emitted). Cumulative ≈+4% through six gates. **v8.4 shipped at G6** — the corpus now spans charter→release. Full-campaign export (datapoints #1–#7, the per-campaign shape from Ask 3) lands at M7.2.
