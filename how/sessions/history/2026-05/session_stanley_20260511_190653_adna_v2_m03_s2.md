...
scope:
  directories:
    - /Users/stanley/lattice/adna/  # upstream LatticeProtocol/adna working tree (renamed to .adna/ at Phase F)
...
- **Destructive upstream-repo edits** (in `/Users/stanley/lattice/adna/` working tree, renamed to `.adna/` at Phase F):
  ...
  - Workspace-level rename (Phase F): `rm /Users/stanley/lattice/.adna` (symlink) + `mv /Users/stanley/lattice/adna /Users/stanley/lattice/.adna` (rename directory).
...
- Workspace state at S2 entry: clean `/Users/stanley/lattice/adna/` working tree (M03 S1 made no upstream-repo touches); `/Users/stanley/lattice/.adna` symlink intact (pointing at `adna/.adna`); workspace router `/Users/stanley/lattice/CLAUDE.md` clean of Agentic-DNA refs (0 hits).
...
- 19:23 — Phase F: workspace-level rename. `rm /Users/stanley/lattice/.adna` (symlink); `mv /Users/stanley/lattice/adna /Users/stanley/lattice/.adna` (directory rename). Verification: `.adna/` is now a real directory not a symlink (V1 satisfied).
- 19:23 — Phase D: V1-V13 verification harness on local workspace. All 13 V-checks PASS (V12/V13 satisfied via Risk 3 mitigation — frontmatter YAML syntax-validation since this session can't launch fresh `claude` from `~/lattice/` to test interactive greeting; operator can verify post-session). Results artifact written at `missions/artifacts/m03_obj5_verification_harness_results.md`.
...
- **Phase F**: workspace-level rename — `rm ~/lattice/.adna` symlink + `mv ~/lattice/adna ~/lattice/.adna`. Real directory now at workspace root.
...
- Workspace-level rename: `~/lattice/.adna` is real directory, not symlink
...
- Workspace-level: `~/lattice/.adna` (was symlink; now real dir) + `~/lattice/adna` (deleted via mv)
