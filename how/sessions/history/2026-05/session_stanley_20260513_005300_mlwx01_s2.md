...
- `~/lattice/.adna/` (template upstream — `c32930e` HEAD preserved; skill under test at `8673383`)
- `~/lattice/CLAUDE.md` (workspace router — 1-line Step 0.3 edit landed at S1)
- `~/lattice/node.aDNA/` (audit baseline + M-LWX-02 outputs — preserved; hard constraint guard via pre/post `git status`)
...
- No in-flight session touching `.adna/` upstream or `~/lattice/CLAUDE.md`
...
- **Decision (D-Smoke)** — Mirror-this-node values used for the 19 interview answers; this node's existing `~/lattice/node.aDNA/HOME.md` is the gold standard for structural diff. Production-comparison fidelity confirmed: 16 visible headers + 0 dangling Home.md + 0 residual `{{VARS}}`.
...
- 00:55 — Sandbox fork: `cp -r ~/lattice/.adna /tmp/sandbox_lwx_01/node.aDNA` + skill_project_fork cleanups (R1-R5 + obsidian state); 4 upstream files survive (HOME.md 7965B, workspace.default.json, skill 18404B, AGENTS.md row); 50 lines with `{{` in HOME.md pre-substitution
...
- 01:01 — Structural diff vs `~/lattice/node.aDNA/HOME.md`: visible section count 16/16 PASS; surfaced source-bloat finding (HTML-comment-internal substitution duplicating content; render-invisible)
...
- 01:03 — Hard-constraint guard: `git status` on `~/lattice/node.aDNA/` clean (HEAD `1032d8d` unchanged); `git status` on `~/lattice/.adna/` clean (HEAD `c32930e` unchanged) — both PASS
...
- [x] Phase A: Hard-constraint guard pre-smoke (`~/lattice/node.aDNA/` HEAD `1032d8d`, clean tree; `~/lattice/.adna/` HEAD `c32930e`, clean tree)
...
- `~/lattice/node.aDNA/` (HEAD `1032d8d` unchanged; M-LWX-02 outputs + M04 audit baseline preserved)
- `~/lattice/.adna/` (HEAD `c32930e` unchanged; upstream commit `8673383` still the pin for the skill under test)
- `~/lattice/CLAUDE.md` (workspace router; S1's 1-line Step 0.3 prompt edit preserved)
