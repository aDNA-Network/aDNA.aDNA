...
4. **Lay down the skeleton** (`who/assets/` subdirs incl. the icon classes + `who/curation/curation_schema.yaml` + the `how/federation/canvasforge/` wrapper + the optional `how/federation/webforge/` wrapper for web-surface generation — federation wrappers live under `how/federation/` per ADR-045; the `webforge/` wrapper is laid down **scaffold-only** and **degrades cleanly when WebForge is absent**, the same optional-with-degradation pattern as `canvas_core` in step 6) and enable **both** CSS snippets under Appearance → CSS snippets.
