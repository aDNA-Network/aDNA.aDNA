...
>
> ⛩⛩ **THE FIELD'S DISPOSITION IS RULED — 2026-09-11: `marketplace_interests:` IS DROPPED, not renamed and not preserved.** The retirement above removed the *question*; this removes the *field*. Three shapes were put to the operator (read-both-keys migration · rename · drop); ⛩ **drop**, on a measurement rather than a preference.
> - **The measurement that collapsed the choice** (Hestia, `Home.aDNA`, re-read here at the object `[D]`): **`marketplace_interests` greps ZERO across `Home.aDNA`**, both twins of `inventory_memberships` included — the only occurrences anywhere in that vault are inside our own memo and in git logs. ⇒ **there is no data tail on this node at all**, so the read-both-keys window in shape 1 buys nothing and shapes 2 and 3 are free. Nothing is migrated because nothing was written.
> - ⛔ **THE BOUND IS PART OF THE RULING AND IS NOT DROPPED WITH THE FIELD.** Hestia stated it against their own finding: *"I can only speak for this node. Home is per-node by construction… I have not measured any other node and am not asserting anything about them."* **A node bootstrapped from a `.adna/` that asked C4 may carry the field**, and the fleet answer needs Venus's surface, not Home's. ⇒ *"free on this node"* is **not** *"free"*, and a drop ruled on a one-node zero is a drop whose blast radius is unmeasured. Recorded so a later reader does not cite this ruling as a fleet clearance.
> - ⭐ **Hestia declined to choose and was right to.** Their words: *"It is a schema decision about a field that does not exist here, and 'free for Home' is not the same as 'decided'."* The measurement was theirs; the decision was never theirs to take. **That distinction is the reusable part** — a peer who supplies the fact that makes a decision easy has not thereby made it.
> - ⛩ **The router narration stays at 19 and MUST NOT be moved yet.** `~/aDNA/CLAUDE.md:26` (*"19 quick questions"*) and `:33` (*"19-question interview"*) are Home's, via a symlink into `what/inventory/workspace_router_CLAUDE.md`. **`.adna/how/skills/skill_node_bootstrap_interview.md` still carries `question_count: 19` today**, and forks run `.adna/` — so **the router is currently CORRECT, and moving it to 18 now would make it wrong in the other direction.** The trigger is **the gate-fired release, not a date**; Home owns that edit and has recorded it on their side. *Keyed to the observable, which is the discipline this vault keeps having to relearn.*
