...
| **ND-3** | Workspace router (`/Users/stanley/aDNA/CLAUDE.md`) uses 4 hyphen-flat references (`science-stanley-adna`, `wga-adna`, `context-commons-adna`, `LAStartupLattice`) when describing GitHub repo URLs of grandfathered vaults. These are accurate (the GitHub repos really are named that way). ADR-009 §Exceptions documents these as grandfathered. | Low (codified) | ADR-009 documents; no template-side action. v3 successor M04-EC handles per-vault rename if operators opt in. |
