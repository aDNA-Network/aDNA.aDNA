...
- **No partner-vault content mutation**. M03 touches only `adna/` (the template repo) + `aDNA.aDNA/` (this campaign vault) + `/Users/stanley/aDNA/CLAUDE.md` (the workspace router, if `Agentic-DNA` references are found there — none currently per Session 1 Phase A spot-check). No partner vault is edited.
...
- **Upstream-vs-vault clarity**. Stages B-C edit the upstream `LatticeProtocol/adna` template repo (cloned at `/Users/stanley/aDNA/adna/`, which becomes `/Users/stanley/aDNA/.adna/` after Path A in-place rename). This is **distinct** from the campaign vault `aDNA.aDNA/`. Session 2 spec-execution must pre-confirm the upstream working tree path + the remote (`LatticeProtocol/adna` post-Stage-A rename) before any `git mv` runs. Confusion between the two paths is the M01 AAR's surfaced risk for this mission class.
...
- **Agent action**: optional remote URL update on local clone (`git -C ~/aDNA/adna remote set-url origin https://github.com/LatticeProtocol/adna.git`) — execute, verify with `git remote -v`
...
- Confirm upstream working tree path: `/Users/stanley/aDNA/adna/` (Path A pre-rename) becomes `/Users/stanley/aDNA/.adna/` (Path A post-rename); record absolute paths in the session activity log
...
- Recommended: backup at `~/aDNA.pre-v7-backup/` (`cp -a ~/aDNA ~/aDNA.pre-v7-backup`)
...
1. **Symlink removal + directory rename** (Path A Steps 1-2): `rm ~/aDNA/.adna; mv ~/aDNA/adna ~/aDNA/.adna`. This is a workspace-level operation; the upstream repo doesn't see it as a commit.
...
2. **`skill_workspace_upgrade.md` updates commit**:
   - **Symlink-step removal** (V10 in verification harness): remove any `ln -s` line that creates `~/aDNA/.adna -> adna/.adna`. Post-flatten, no symlink exists.
   - **Step 3 alternative — template-based bootstrap** (ADR-007 §Workflow integration): add an alternative Step 3 path: if no workspace router exists at `~/aDNA/CLAUDE.md`, copy `.adna/how/templates/template_workspace_claude.md` → `~/aDNA/CLAUDE.md` as the starting point.
...
| III.aDNA airlock spec version (v0.2.0) | `/Users/stanley/aDNA/III.aDNA/what/artifacts/iii_airlock_standard_spec.md` | Obj 1 ADR-008 federation_ref pin |
...
- [ ] **`Agentic-DNA` references swept** in `.adna/` (1 known reference in `.adna/CLAUDE.md`; any others found post-grep also swept); workspace router `/Users/stanley/aDNA/CLAUDE.md` re-verified clean
