...
- **LIVE capture (`adna.network`) — done 2026-06-18.** Lighthouse on 7 flagships + 7 group-reps: Perf 100 / A11y 100 (home 96) / SEO 100 — but **Best-Practices capped at 92 on EVERY live route** (100 local) ⇒ a **Vercel deploy-layer issue** (headers/source-maps), systemic, not source. `/learn/concepts/*` **CLS 0.156 live (0.134 local)** — a real layout shift on the concept template (axis F/H). Live link-check: all 200 except `/commons` **404**; homepage external link = `github.com/aDNA-Network/aDNA` (200, canonical) — no stale `LatticeProtocol` link renders.
- **Deploy gap (first-class finding).** Live ≈ E4 c159 + install-truth hotfix; local HEAD ≈ c165. Concrete: **`/commons` missing live (404)**; home **5 live sections vs 6 local** (the c165 "Join the network" §5 undeployed); `/vaults` + `/vaults/graph` carry stale content (c162 glyph-language undeployed); live vault links still pre-rename (`/vaults/TaskForge.aDNA`, `/vaults/CanvasForge.aDNA` — renamed at pt08/pt09 after this deploy → reinforces the pt19-owned class).
- **Coverage (no silent caps):** captured = local flagships LH + full-route local `@audit` both-modes (114/115) + local group-sample LH + **live** flagships + live group-sample LH + live link-check + deploy-gap diff. Not exhaustively captured (by design — sample, not 163×2): per-route LH for the ~149 non-sampled pages; a live per-route axe sweep (live = link-status only; dark-mode axe covered by the local both-modes sweep). Evidence: 28 JSONs in `site/evidence/wadna_baseline_2026_06/` (gitignored).
