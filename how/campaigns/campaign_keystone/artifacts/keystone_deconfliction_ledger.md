...
| Store (MinIO/AIStor) | `Store.aDNA` | **Yes** → data-plane, Venus placement | **VERIFIED ABSENT** (MinIO exists only as latlab L3 infra; no vault, no remote). Lattice Storage Federation. Seed net-new. | P2 |
| Groupware (Stalwart JMAP) | `Groupware.aDNA` | **Yes** → data-plane, Venus placement | **VERIFIED ABSENT** (no Stalwart/JMAP anywhere). Unified messaging. Seed net-new. | P2 |
