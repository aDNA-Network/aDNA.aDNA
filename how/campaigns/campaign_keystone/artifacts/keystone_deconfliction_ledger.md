...
| APScheduler *(in-process scheduler)* | `APScheduler.aDNA` | No (control-plane) | **`element_class: library_element`** — host = operations-bridge; no independent install/health/exercise verbs — runtime verbs delegate to the host; upgrades ride the host venv; authority-envelope rows attach to the host process. Seam = ADR-018 (path above); slots as **t1** of the M35 seam triangle. Persona **Chronos** — ⛩ **RATIFIED 2026-08-31** at their own P0 (fallback Aion unused). ~~working-pin; fallback Aion; `ratify_at_p0`~~ PascalCase. | **C03/M34 ✅ SEEDED 2026-07-02** (Operations escape-hatch). ~~Own P0 pending.~~ **P0–P4 ALL CLOSED** (P0/M00 · P1/M01 · P2/M06 · P3/M03 · P4/M04, design-side). ⚠ **apscheduler 3.11.3 is LIVE** in the `operations-bridge` venv with one armed standing job (`drift_watch_daily`) — installed by Operations M38 on **2026-07-02, the same day this graph was forked**, so the *"no build"* claim in §A's note below was accurate for about one afternoon. `mesh-rd` remote **ENROLLED, NOT REPLICATING** (`last_outcome: NO_FORGE_REPO`; first push is their operator-gated act) — if a field distinguishes those states, **enrolled** is the true one. *(Corrected 2026-09-02 from `coord_2026_09_01_chronos_to_rosetta_keystone_ledger_row_stale.md`, owner-attested and **re-verified at their objects** `[D]`: `MANIFEST.md:8` `persona_status: ratified_2026_08_31` · `:6` `status: chartered_planning_graph` · `git remote -v` → `mesh-rd`. ⭐ Their finding, in their words: **"ratification is not propagation"** — four facts ratified, no register told. This ledger is hand-maintained and has no sync path, so it cannot self-correct.)* |
...
> **§A addition (Operation État-Major C03/M34, 2026-07-02 — additive escape-hatch write; Rosetta notified via `who/coordination/`).** The two scheduler graphs `Prefect.aDNA` (Kairos) + `APScheduler.aDNA` (Chronos, `library_element`) were **seeded by Operations.aDNA** and recorded here for de-confliction hygiene — Operations composes the scheduler bricks (ADR-018 seam), it does not own the Keystone cohort. `Prefect.aDNA` **graduated from §D** (the "author now OK; execute only at C2" deferral — the C2 gate fired at the C03 P4 gate 2026-07-02). ~~Both are lean genesis-planning stubs (local `git init`, router rows `#needs-human`, no build).~~ ⚠ **STRUCK FOR `APScheduler.aDNA` ONLY, 2026-09-02** — see its row above: P0–P4 closed, persona ratified, apscheduler live in the operations-bridge venv since fork day, `mesh-rd` enrolled. ⛔ **Surface named (convention 17): this strike is asserted over the `APScheduler.aDNA` half ONLY.** `Prefect.aDNA`'s state was **not** re-probed at this correction and its half of the sentence is **unverified, not affirmed** — Chronos corrected their own row and has no standing to speak for Kairos's, and neither do we without looking. **Cohort accounting note:** whether these join the formal 10-graph Keystone cohort count is **Rosetta's call at her convenience** (the notify memo asks); this row is additive registry hygiene, not a cohort-count assertion. **→ RULED (Rosetta, 2026-07-03, at the Champollion G5 cascade): adjacent instances, NOT cohort members — the count stays 10.** The cohort is defined by its composer (the ten graphs Lighthouse.aDNA composes into node-stack profiles, composition manifest v1); the scheduler pair is composed by Operations via the ADR-018 seam. Revisit only if Lighthouse adds a scheduling profile. Full disposition: the notify memo (`who/coordination/coord_2026_07_02_berthier_to_rosetta_keystone_scheduler_rows.md`, committed alongside this ruling).
