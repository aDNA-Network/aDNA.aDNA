...
**Ratification-time adjustments (2026-07-08, operator) — 4 folded:** (1) **evidence completed pre-ratification** ([[evidence_completion]]) — **T2 improved**: the *standard* is genuinely credible (MIT · semver v7→v8.6 · working CI · a real `Canvas.aDNA` code lib), and the 6 persona hubs (never opened by the first red-team) are real assets that under-claim honestly (axe-clean, mechanism-level); the gap is the *network/protocol* claims outrunning public evidence + a **broken public Lattice-Protocol link** (→ a private/404 repo). (2) a **graph de-risk spike** (Phase 0.5). (3) a **decade-1 / ship-and-measure** framing. (4) the **3 decision points pre-resolved**.
...
> **Decade framing (ship-and-measure).** **Decade 1 — firmly committed = P0 · P0.5 · P1 · P2** (evidence · graph-direction · storytelling · the legible graph). **Decade 2 — provisional, re-planned after Decade 1 deploys + is measured = P3 · P4 · P5.** Improvements ship **live per phase**, not at the end.

### Phase 0 — Evidence completion (unblock the T2 verdict)
| Mission | Title | Sessions | Deps | Status |
|---|---|---|---|---|
| M0.1 | Surface-audit + link-graph crawl (persona hubs · `/org-context-graphs` orphan · `/how/*` · `/vaults/[slug]` sample) — re-confirm T2 on real evidence | 1–2 | — | **done** (2026-07-08) |
| M0.2 | Verify-live measured pass (axe · install-truth · version · OG · orphan · console/status) via the T0 harness | 1 | — | **partial** (surfaces+axe done; live Lighthouse/CWV remaining) |
**Exit gate (operator):** T2 re-confirmed on the full surface set; measured baselines captured. **⚡ Largely DONE pre-ratification** ([[evidence_completion]]): surfaces opened + axe-measured (hubs 0 violations; vault-detail 3× ≈40 pages = EV4); install/version/OG confirmed; `/org-context-graphs` → **RETIRE** (orphan + no-JS-render-broken, EV5). Remaining = a real Lighthouse/CWV run. *(B1, B2 + EV1–EV8.)*

### Phase 0.5 — Graph feasibility + design-comp spike (de-risk the flagship early)
| Mission | Title | Sessions | Deps | Status |
|---|---|---|---|---|
| M0.5 | Throwaway interactive **graph prototype** (split connected [15/14] from unconnected [53]; pan/zoom/hover; D3 or SSR) + **2–3 `artifact-design` comps** for operator selection + an ADR-032-fit assessment | 1–2 | P0 | planned |
**Exit gate (operator):** a **chosen graph direction** (from the comps) + feasibility confirmed + ADR-032 register reconciled (default: **composed focal unit** — warm scene *and* the real graph as one unit). *Fail-fast on the hardest, flagship item before P2's full build; the P1 copy wins run in parallel.* *(De-risks B3/P2.)*

### Phase 1 — Storytelling core
| Mission | Title | Sessions | Deps | Status |
|---|---|---|---|---|
| M1.1 | **"Who's behind this"** credibility surface — real FA + Wilhelm partner + one impact case + the **governance roadmap** + **humanization** framing; **⚡EV1** fix the broken Lattice-Protocol link + honestly reframe protocol/network as *opening progressively*; **EV2** swap fictional use-cases → the real subnetworks (WGA · Context Commons · WilhelmAI · Rare Archive); **EV3** name a real human/security contact; **foreground the real public assets** (MIT · CI · `Canvas` code lib · Wilhelm · the dated verifiable logs) | 2–3 | P0 | planned |
...
**Exit gate:** home + credibility surface pass the ranker; both skeptics flip to WOULD-TRY/WOULD-ENGAGE; the 60-sec-newcomer read passes. **Operator-strategic flag (beyond the site — the single biggest external unlock):** make `lattice-protocol` + the whitepaper **public** so the protocol/network claims become externally verifiable. *(B4, B5, B10-home, EV1–EV3, EV8.)*
...
| M4.2 | `/reference` + `/learn` IA — lead-with-spec + **version stamp** (EV6: display gap; data is v2.5-correct) · group by genre · reconcile the two navs · mobile reflow · ordered path · **EV7** bridge `/compliance` to a named regime (SOC 2 / ISO 27001 / EU AI Act) | 1–2 | — | planned |
...
| M5.2 | A11y + perf + reach — real axe remediation (incl. **EV4** vault-detail 3× ≈40 pages) · CLS/perf budget · OG imagery · **i18n + low-bandwidth** posture (plan) · site legal/privacy notice · deferred long-tail (B13) | 2 | P4 | planned |
...
| 2 | **P0.5** exit | **ADR-032 reconciliation** — real-graph-as-hero vs illustrative register | **RESOLVED-default → composed focal unit** (warm scene + real graph = one unit; confirm at the P0.5 spike) |
| 3 | each phase | Deploy/ship ratification (deploy cadence: **ship per phase** + credibility-hotfix path) | pending (recurring) |
| 4 | P1 | Biodiversity — feature a subnetwork vs. state as a mission direction | **RESOLVED-default → mission direction now**; feature a subnetwork when a real partner lands (never fabricated) |
...
## Timeline
| Phase | Missions | Sessions | Decade |
|---|---|---|---|
| P0 evidence | M0.1–M0.2 | ~1 (**mostly done pre-ratification**; live Lighthouse remains) | **1 (firm)** |
| P0.5 graph spike | M0.5 | 1–2 | **1 (firm)** |
| P1 storytelling | M1.1–M1.2 | 3–4 | **1 (firm)** |
| P2 graph | M2.1–M2.2 | 3–5 | **1 (firm)** |
| P3 actionability | M3.1–M3.2 | 3–4 | 2 (provisional) |
| P4 docs/de-jargon | M4.1–M4.2 | 3–4 | 2 (provisional) |
| P5 craft/a11y/perf | M5.1–M5.2 | 3–4 | 2 (provisional) |
| **Total** | **12 missions** | **~13–20 (calibrated 11–16)** | — |
...
- **Evidence pass largely complete** ([[evidence_completion]], 2026-07-08) — the campaign effectively begins at **P0.5**. **Decade 1 (P0–P2)** ships + is measured before **Decade 2 (P3–P5)** is re-planned.
