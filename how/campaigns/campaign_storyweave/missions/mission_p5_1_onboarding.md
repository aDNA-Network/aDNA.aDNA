...
A real **~30–60 s screencast** — *clone → `claude` → the agent orients from `STATE.md` + does one real task* — on home ("How it Works" section, **not** the hero) + `/get-started`, replacing the static ASCII mock as the honest poster/fallback. Files: new asset + a lazy-loaded player component. **Hard gates:** hold home **CLS 0 / TBT 0 / Perf ≥ 99** (lazy-load + `poster` + **no autoplay-with-sound**; optimized GIF / `<video muted playsinline>` / `asciinema` cast); no-JS + a11y (captions/controls; ASCII mock = honest fallback); a **real recording, never staged** (Movement-Skeptic). **Waits for O3.** ⛩ **O3 done + ratified 2026-07-12** (front_page_doctrine §10 durable; **R1 format de-risked → lightest honest product-in-action: `asciinema` cast / muted-lazy `<video>`, poster=ASCII, below LCP, never autoplay-with-sound**). **The demo asset (the real recording) + its final format are operator-DEFERRED to an in-person dev-rel build session (week of 2026-07-13).** Turnkey build brief + recording runbook (placement · P1.6 pattern · full gate/CSP/perf checklist · demo script · both recording paths): **[[m5_1b_demo_build_brief]]**.
...
| **O4** | **M5.1b build + gate + ship** — the ~30–60 s demo screencast (**asset + format operator-DEFERRED to the in-person dev-rel build, week of 2026-07-13; turnkey brief [[m5_1b_demo_build_brief]]**; O3 recommends `asciinema`); **hard** perf/CLS gate + no-JS/a11y baseline + real-recording (Movement-Skeptic) → operator ship-gate → deploy | M5.1b live | ⛩ operator |
...
> **Progress (O3, 2026-07-12):** **O3 DELIVERED** — the R1 ≥6-exemplar install-forward reference pass ran ([[skill_reference_inspection]]): 6 exemplars inspected (Hermes·Val Town·Raycast·Vercel·Bun·Astro; both tonal poles; headless captures 200 ×6, dark+light). Outputs: 2 new durable artifacts ([[site_bun]]·[[site_astro]]) + install-forward addenda on the 4 existing + refreshed durable [[_reference_set]] §Install-forward synthesis; **R1–R5 promoted → [[front_page_doctrine]] §10 (PROPOSED, pending ⛩ operator ratification)** with per-rule provenance. **R1 demo-format de-risked:** the CLI peers (Bun/Astro) hero benchmark/code, NOT video; only Hermes (an agent product) heroes "video of usage" → **R1 = lightest honest product-in-action** (`asciinema` cast or muted-lazy `<video>`/GIF, poster = ASCII fallback, below LCP; never autoplay-with-sound), holding home CLS 0 / TBT 0 / Perf ≥99. Skill advance gate MET (6 Tier-A-full · both poles · functional spread · provenance on every rule). Doctrine/research pass — zero `site/` edits, no build, no deploy. **⛩ §10 RATIFIED durable (operator, O3 gate 2026-07-12) → O4 (M5.1b R1 demo build) unblocked.** O3 committed LOCAL (push operator-gated, SO-11). Session `session_stanley_20260712_183151_storyweave_p5_o3_reference_pass`.
