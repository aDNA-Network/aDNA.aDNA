...
decade: 2
owner: stanley
status: completed     # SHIPPED + LIVE 2026-07-12 (dpl_HicFkr9DmPPynDX1r1LzADxdD5Co; adna.network 200; live T0 axe-0). O0–O3 built · O4 folded→M5.3 · O5 gated (359 green, ranker 4.5) + shipped + pushed (b3a396d..2c93864, gitleaks clean) · O6 AAR done. Baton → M5.3.
mission_class: build
...
## AAR (SO#5) — M5.2 close, 2026-07-12

- **Worked.** The *gate-every-fix* discipline paid off: **gate-24-copy-craft** + **gate-25-token-discipline** lock each fix durably (M5.1a's "extend the gate that guards it" lesson, generalized). `excerpt()` authored **word-boundary-only + idempotent on clean text** → the ~11 index pages' truncated cards went word-safe with a Unicode `…` while already-clean cards stayed byte-identical (and `AGENTS.md`/`3.8+` internal dots survived — a sentence-splitter would have mis-cut them). `palette.ts` single-sourced the Mermaid palette **byte-identical** (verified both themes via T0). The `/design-system` page was authored **token-pure** so it passes its own gate-25. The **home-safe boundary held** — home Perf 100/CLS 0/TBT 0 unchanged. **Ranker 4.5 first pass** (all four lenses 4.5), shipped + live first attempt (`dpl_HicFkr9DmPPynDX1r1LzADxdD5Co`; adna.network 200; live T0 axe-0).
- **Didn't (traps caught).** (1) The p5_replan's **"gate-14 token discipline" label was a misnomer** — gate-14 is a *repo/publisher-drift* lint, not a hex gate; verified on disk → authored gate-25 instead of trusting the label. (2) The `/design-system` page **surfaced a LATENT AA bug that had been shipping**: CardGrid badges (`updated/deprecated/experimental`) used raw `--color-info/warning/accent` as text on a 15% tint → **1.9–2.45:1 in light mode**, live on the 2 index pages that wire `badge:`. Caught only because a showcase page forces every component variant onto one gated surface. Fixed via the proven status-pill `color-mix(…, --color-text)` darkening. (3) The **Plan subagents flaked twice** (0-tool boilerplate returns) → fell back to reading files directly. (4) A shell `${VAR:-MISSING}` fallback **echoed the (throwaway) Vercel token once** — logic bug.
- **Finding.** The residual hardcoded hex is **all deliberate** (terminal chrome, macOS traffic-light literals, AA-tuned status/btn colors, the always-dark hero scrim, the B1 orphan) — **not drift**. So "kill hardcoded hex" was partly a misdiagnosis; the durable win was **inventory + lock** (gate-25 as a non-regression fence), not a cleanup. General lesson: **a design-system showcase page is a high-yield latent-bug detector** — it co-locates every component variant on one gated surface, exposing contrast bugs the components hide when rendered sparsely.
- **Change (for next time).** (a) When a plan cites a gate **by number**, verify the gate's actual assertion on disk before trusting the label — the gate-14 misnomer would have mis-scoped the work. (b) Redact secrets with `${VAR:+SET}` **only** — never a `${VAR:-…}` fallback, which echoes the value when set. (c) The Plan subagent type is flaky on this node right now — do design-grounding by direct file reads, not Plan agents.
- **Follow-up.** (1) **O4 folded items → M5.3** (Axis-4 graph cohesion + hero keyboard-focus; both home-touching). (2) **hero-letterbox PNG re-cut** (asset; 4 doc-heroes; imagen pipeline). (3) **51-file frontmatter meta regen** (`scripts/transform-content.mjs` — cleans detail-page `<meta>` too). (4) `excerpt()`/`firstSentence()` consolidation (flagged in `text.ts`). (5) HomeHero canvas/scrim palette + stability-color token dedup (gate-25 allowlist residuals). (6) image-palette build check. **Next = M5.3** (a11y/perf/reach, de-risked) → then the **capstone ranker ≥ 4.95** graduates the campaign.
- **Token / tier.** `executor_tier: sonnet`-class mechanical craft driven under **opus** judgment (the gate-14-misnomer catch, the latent-AA-bug fix, the design-system page design, the ship-gate). Budget: est. ~60–90 kT; **actual ≈ 1 session**, within envelope. Deploy `dpl_HicFkr9DmPPynDX1r1LzADxdD5Co`; commits `b3a396d..2c93864` pushed origin/main (gitleaks clean).
