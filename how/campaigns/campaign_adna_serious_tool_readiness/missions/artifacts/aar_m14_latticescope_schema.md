...
3. **Sanitized-cwd path encoding empirically validated**: `/Users/stanley/lattice/aDNA.aDNA` → `-Users-stanley-lattice-aDNA-aDNA` (both `/` and `.` map to `-`). Confirmed by inspection of `~/.claude/projects/` at M1.4 S1; resolver pattern wired in hook + Python script.
