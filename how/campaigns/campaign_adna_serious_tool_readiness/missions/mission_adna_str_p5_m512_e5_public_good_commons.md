...
3. **O3 — Stage 5 build** (cycles 161–164): the `/commons` showcase page (ethos hero → featured-subnetwork grid → connect CTA → contributors/activity → govern/trust close); the subnetwork detail treatment + the **"connect to a subnetwork"** affordance; the **first social surface** (contributors/attribution/activity over the registry — honest MVP, horizon note); the homepage **§5** hand-off (the E1 tease now points at a live surface) + nav entry + cross-links.
