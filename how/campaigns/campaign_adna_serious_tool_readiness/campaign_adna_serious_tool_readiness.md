...
| M1.3 | Token audit + measurement protocol activation (absorbs v2 M09) | 2-3 | M1.2 | **in_progress** (S1 closed 2026-05-18T20:00Z; **S2 closed 2026-05-18T20:10Z+ — PostToolUse hook LIVE + self-test PASS + Type B+C+D dataset + α/β/γ/δ ranking + convergence-model verdict Mid-magnitude + top-3 M2.1 queue**; cumulative 6/7 deliverables landed; S3 mission close non-destructive + operator-discretionary) |
| M1.4 | LatticeScope.aDNA v0.1 schema activation for observability (absorbs v2 M10; bootstraps `LatticeScope.aDNA/` sub-vault if needed) | 2-3 | M1.3 | planned (opens at M1.3 close) |
