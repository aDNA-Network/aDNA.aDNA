...
| **P7 ✅** | Ship & handoff — **WORK COMPLETE 2026-07-03; G7 RENDERED (operator close gate)** | G6 fired ✅ | M7.1 post-ship walk **green** (0 blockers) + handoff packet w/ the v8.5 queue; M7.2 ops-retro (→ pattern §2.6) + telemetry corpus export (#1–7) + campaign AAR + close splash; DP4 READY (operator fires). G7 = campaign close (operator gate) |
...
## 8. Completion (P7 — pending G7 ratification)

**Work complete 2026-07-03.** All 24 missions closed; the base layer's terminal deliverable (**v8.4** image · **v2.5** standard) shipped at G6. P7 verified-in-the-wild (M7.1 post-ship walk: **0 blockers**, 2 majors → v8.5) and closed (M7.2): ops-retro → [[../../what/patterns/pattern_model_tiered_campaign_execution|pattern §2.6]] · [[artifacts/telemetry_corpus_champollion_export|telemetry corpus export #1–7]] · [[artifacts/aar_champollion|campaign AAR]] · [[artifacts/handoff_packet_v8_4|handoff packet]] (v8.5 queue). **Definition-of-Done: MET** (AAR §3; the one operator-reserved residual is DP4 firing).

**G7 = the campaign-close operator gate** ([[../../gates/champollion_p7_gate|champollion_p7_gate]]) — renders campaign → `completed`, residual-thread routing, and the final P7 push batch. The close splash + decisions live in the gate. **Status stays `active` until the operator fires G7 — do not auto-advance.**
