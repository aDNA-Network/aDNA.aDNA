...
> ⚠ **Correction (M4.3 review, same day)**: the counts above are FILE counts, but the router's sentences name SEMANTIC units — the semantic census is **15** `*.lattice.yaml` example definitions (the "20" includes AGENTS.md + 4 `.canvas` templates → **the router's "15" is CORRECT; N-02 RETRACTED**) · **26** `template_*.md` reusable templates (not 27 — AGENTS.md is an index, not a template) · **26** `skill_*.md` (of which 24 `skill_type: agent`). N-03/N-04 stand with corrected true-counts. Both the M4.1 builder AND the fable verifier counted files — same method, so the "independent" check replicated the error (semantic-census corollary now in patterns/AGENTS.md; meta-finding F-CHM-212).
...
| N-02 | ~~paper-cut~~ **RETRACTED** | `image:CLAUDE.md:137` | ~~Router says "15 example lattice definitions"; the image has **20**~~ **RETRACTED at M4.3 review**: the router is CORRECT — 15 `*.lattice.yaml`; the "20" was a file count including AGENTS.md + 4 `.canvas` templates (semantic-census corollary; F-CHM-212). |
...
| N-02 | **RETRACTED** (was (c)) | `image:CLAUDE.md:137` | Retracted at M4.3 review — the router's "15 example lattice definitions" is semantically CORRECT; no fix needed (fold-stub item marked retracted). |
