...
| 2 graduation seeds (~~3rd-instance gated~~ **ALREADY 3/3**) | `skill_documentation_layout_props_additive_extension` (**3/3**, graduated cycle 107) · `skill_inline_svg_raw_import_currentColor_inheritance` (**3/3**, graduated cycle 108) — both **GRADUATED 2026-05-27** (v8 M5.3 S3; OoB said 2/3 & 1/3 — stale). **M3.2 re-derived 2026-07-02: backlog filed** [[idea_seed_skill_documentation_layout_props_additive_extension\|layout_props]] · [[idea_seed_skill_inline_svg_raw_import_currentcolor_inheritance\|currentColor]]) | **✅ RATIFIED at G3 D2c** (2026-07-02) — authoring rides **M4.2** as in-window riders |
| Registry exercise (readiness ≠ registered) | No lattice has ever been **published** to a registry from this vault (M3.3 D8 finding; readiness metadata complete on all 19 example lattices). Not launch-critical. | **Deferred at G3 D6.5** — owner **Rosetta** · trigger **first post-launch maintenance window** (publish ≥1 example lattice end-to-end, exercising `skill_lattice_publish`) |
| latlab CLI non-operational on this node | `/opt/anaconda3/bin/latlab` → `ModuleNotFoundError: No module named 'cli'`; no local registry store (M5.3 environment finding, [[adoption_story_record\|adoption_story_record]] §1). Not a content defect — the teaching labels registry steps TAUGHT-AS-DESIGN on durable grounds regardless. But **the registry exercise above needs a working CLI first.** | **Deferred at G5 D5.2** — owner **Rosetta** · trigger **the same post-launch maintenance window** (surface to the `latlab` owner then; fix precedes the registry exercise) |
