...
| 2026-06-28 | M4 | Improve-phase division: **delegate the mechanical sweep → verify in the main loop** | **graduate (technique)** | the 15-file exact-checklist sweep ran as a subagent; correctness came from main-loop grep-residual + `git diff` + the do-not-touch guardrail — the right split for voluminous-but-precise Improve work. |
| 2026-06-28 | M4 | **flag-not-fix discipline** for out-of-slice findings | **graduate (doctrine)** | the broad grep found the drift class beyond the chartered slice; the charter's Subject-B bound + Risk Register said flag-not-fix → emitted `idea_vault_wide_currency_sweep`. Bounding worked — it prevented scope-creep mid-pilot. |
...
| 2026-06-28 | M4 | `skill_iii_cycle`'s Improve step fixes *content* but doesn't model **repairing the instrument that missed it** — A-01 shipped the proof-links fix *with* its G20 gate (the C-027 closed loop). | an Improve-phase step that closes Inspect's instrument-gaps as part of the fix (fix + gate = one deliverable). |
| 2026-06-28 | M4 | **live-probe vs. local-existence** — the local `.adna/` is the frozen pre-v8.0 legacy, so a local existence gate cannot verify the v8.0 proof-links; the released image diverges from the local check target. | a fidelity campaign on a released-vs-local-divergent artifact needs a **live-probe** instrument (HTTP-200 in CI), not just local existence. |
...
| 2026-06-28 | M4 | **Improve ADDS instrument coverage, not just content fixes** — re-measure 302→**304** (the A-01 G20-manifest claim) with zero regression; B1/B2 flipped FAIL→PASS; marquee A/B inversion resolved. *Pattern lesson: the harness should GROW across an III campaign — a green re-measure with a higher gate count is the healthy signal, not a flat one.* |
| 2026-06-28 | M4 | **the bounded-subject decision is validated by what it excluded** — the same v2.1/`LatticeProtocol` drift extends vault-wide (context library, docs guides, ecosystem specs, even the spec doc); the bound prevented an unbounded sweep. *Pattern lesson: a currency/fidelity III campaign should expect "the drift is bigger than the slice" and carry a flag-not-fix + follow-on discipline.* |
