...
> **STATUS 2026-06-23 — SHIPPED LIVE as part of the combined v8.1.** The operator combined this F1 §5 body-completion with the Cornerstone Obsidian parity + fork-reseed fix into one **v8.1** release ([[adr_038_combined_v81_release]]); it was **assembled + tagged (`32b3793` / `v8.1`) and PUSHED LIVE by the operator** to public `aDNA-Network/aDNA` (verified green pre- and post-push; step-e local `.adna` synced, step-f post-push smoke GREEN = M07 P4 met), shipped **decoupled** from the keystone (ADR-038 D5 — supersedes the §10 DP3 launch-coupling question). The §2 delta applied exactly as specified (diff-verified at fire time: only the 4+3 lines moved). This package remains the canonical F1 spec; the combined run is recorded in [[adr_038_combined_v81_release]] §Ship Path + [[coord_2026_06_23_rosetta_to_seshat_cornerstone_v81_landed]].
