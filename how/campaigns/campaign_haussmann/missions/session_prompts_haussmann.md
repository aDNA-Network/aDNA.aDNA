...
| **P3.1** | *(post-DP6)* You are Rosetta in ~/aDNA/aDNA.aDNA. Execute `mission_haussmann_p3_1_md_twins.md`. Twins derive from the same single-source content as HTML; fix the old .md link targets to the new twins. |
| **P3.2** | *(post-DP6)* You are Rosetta in ~/aDNA/aDNA.aDNA. Execute `mission_haussmann_p3_2_registry_json.md`. The endpoint serves the sanitized projection only; versioned URL; honest-absent nulls. |
...
| **P3.5** | *(post-DP6)* You are Rosetta in ~/aDNA/aDNA.aDNA. Execute `mission_haussmann_p3_5_proposal_process.md`. Halt at O0 for ADR-055 ratification; proposal #1 must traverse real states. |
