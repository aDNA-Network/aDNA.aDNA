...
| **P3.3** | ⏸ **O0 ✅ + O1 ✅ + O3 ✅ (REDUCED) 2026-08-21 — STILL OPEN AT ⛩ O2.** The homepage machine-door block shipped in its **AC2-deferred** form (llms.txt + `.md` twins + `/api/registry.v1.json` only; NO server, NO install line, NO `/.well-known/mcp.json` — that descriptor is still 404 and stays 404 until the publish). Package `adna-mcp-server` is built + red-tested at `mcp/` and **unpublished**; `machine_eye` item 11 is **UNMOVED** and reported so. ⛔ **O2 is not just un-GO'd — it is not performable on this node** (no npm identity: `whoami` ENEEDAUTH, no `.npmrc`, no token, no broker row); it needs an interactive operator `npm login` before any GO means anything. ⏸ **O3 built but NOT DEPLOYED** — strike this when it ships. ⬅ **RESUME HERE at O2**, then the discoverability limb + the homepage server line, which ride the publish. *(post-DP6)* You are Rosetta in ~/aDNA/aDNA.aDNA. Execute `mission_haussmann_p3_3_mcp_server.md` (needs P3.1+P3.2). The self-conformance sentence must pass the editorial gate. |
