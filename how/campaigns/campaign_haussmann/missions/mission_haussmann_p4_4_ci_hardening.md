...
| B2 | Unlighthouse scheduled sweep over the **CI-built artifact** + budget config; **provenance tested by hash**, not asserted (AC4 as replaced). ⚠ **Hash the bars' SOURCE, never a mirrored file's presence** — see AC4's amended row; the mirror is withdrawn (F-e) | sweep + budgets | ⊳ D-E (Vitruvius) — ~~**the ask is STAGED, not delivered**~~ **DELIVERED 2026-08-27** (`44c4d79`, P4.5b O4; `ack_required: true`), **no reply yet** `[D]` — surface named: `who/coordination/` here **and** `WebForge.aDNA/who/coordination/`. B2's shape depends on their answer. ⛩ **B2 IS PROPOSED FOR A SPLIT** into **B2a** (the sweep — no external dependency) / **B2b** (budget provenance — gated), on the reachability line this mission's own P4.4a/P4.4b split used: see `ac_amendment_proposal_p4_4b.md` FINDING 8 |
