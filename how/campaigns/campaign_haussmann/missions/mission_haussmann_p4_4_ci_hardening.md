...
| B2 | Unlighthouse scheduled sweep over the **CI-built artifact** + budget config; **provenance tested by hash**, not asserted (AC4 as replaced). ⚠ **Hash the bars' SOURCE, never a mirrored file's presence** — see AC4's amended row; the mirror is withdrawn (F-e) | sweep + budgets | ⊳ D-E (Vitruvius) — **the ask is STAGED, not delivered**; B2's shape depends on their answer |
