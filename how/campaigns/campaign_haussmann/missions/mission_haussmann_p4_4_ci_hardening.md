...
| # | Objective | Output | Gate |
|---|---|---|---|
| ✅ **B0** | ~~Visual-regression lane…~~ **DONE 2026-08-26** (`1816993`). `gate-49` — G49a frame · G49b mask liveness · G49c pinned mask arithmetic · G49d theme control · 24 baselines, generated AND compared in `mcr.microsoft.com/playwright:v1.59.1-noble`. Suite **633 → 659** derived. **AC1 ✅** | gate + red-test | — |
|  |  ⭐⭐ **V1 IS 7/7 (5 red + 2 controls) AND IT FOUND THREE DEFECTS IN THE GATE BEFORE THE GATE FOUND ANYTHING IN THE SITE.** (1) **The threshold was too loose** — `maxDiffPixelRatio: 0.002`, commented *"deliberately TIGHT"*, let a plainly visible `h1` regression through, because one heading's glyphs are a vanishing fraction of a 7,597 px capture; **AC1 itself supplied the fix** (a tolerance absorbs non-determinism, and same-container comparison removes the only source) ⇒ **`maxDiffPixels: 0`**. ⚠ **Both tolerances were first drafted too loose, same author, same sitting** — the mask budget was 0.40 % against a 0.0716 % worst case before being measured and pinned to 0.15 %. (2) **Two mutation cases were INERT** (bare selectors losing to Astro's scoped `[data-astro-cid-…]`) — **and one was a CONTROL, so it reported success**; *a control that passes for the wrong reason is worse than no control*. (3) **`home` was unstable and the first three explanations were all wrong** — the cause is **the capture perturbing the page** (`fullPage` resize → the hero's `ResizeObserver` redraws the canvas). ⭐ **Every one of the three had a MASK available as a fix and every mask would have gone GREEN** — over-masking arriving disguised as a flake remedy, *within an hour of authoring the gate against it*. Taken instead: `reducedMotion: 'reduce'` (**the site's own mechanism**) and the canvas **hidden, not masked**, so the SSR SVG beneath **is asserted**. ⚠ Limitation on the gate's face: the baselines assert the **reduced-motion** render; the animated constellation is **not covered and not claimed**. ⚠ Baselines are **15 MB / 24 files** — named, not absorbed. | | |
| ✅ **B1** | ~~Field instrument **shipped in the tree AND EMITTING**…~~ **DONE 2026-08-28** (`9c8d79b`). `web-vitals@6` emitter wired in `BaseLayout` — same-origin, **ZERO-NETWORK by construction** (three constraints each independently forbid a transport today: CSP `connect-src 'self'` · `output: 'static'` has no collector · gate-42 reds any failed request); **EMITS observed live**: TTFB + FCP on one page load, buffered to `window.__adnaVitals` + `adna:vital` CustomEvent. `gate-50` (shipped · emits · zero-network · event channel) red-proven **6/6** (4 mutations + 2 controls; V4 `[asserts AC2]` ✅). `/privacy` gained §performance **in the same commit, BEFORE ship, per that page's own written commitment**; changelog entry 2026-08-28. **AC2 ✅ ON-BUILD.** ⛩ **THE OWED-LIST (AC2's face, not claimed):** (1) the operator's **dashboard action** — enable Vercel Speed Insights for the project (toolkit A8's p75 source); unblock = operator act. (2) the **transport wiring** — its own follow-up gate once (1) exists; `/privacy` updates FIRST again. (3) the **first p75 reading** — unblock = (1) + (2) + deploy + **traffic accumulation**. | wired instrument + owed-list | ⛩ operator (enable) |
| ✅ **B2a** | ~~Unlighthouse scheduled sweep…~~ **DONE 2026-08-28** (`f852060`). `unlighthouse-sweep.yml` — weekly (Tue 07:43 UTC) + `workflow_dispatch`, over the **CI-built artifact** (`npx astro build` → local preview), budget-failing, **no `continue-on-error` anywhere**; **JOINS `gates-${{ github.ref }}`** exactly as gates.yml's own contract (lines 142–146) requires — `cancel-in-progress: false`, so it **queues behind** a running gate lane rather than cancelling it. `gate-51` contract gate (schedule · joined group · fails-loudly + pinned runner + **transcription-drift detector** binding the bar to gate-19's `perfMin`×100 same-diff · build discipline) red-proven **7/7** (5 mutations + 2 controls). **V3 `[asserts AC3]` ✅ demonstrated live**: EXIT=1 at bar 100 with per-route ERROR lines, EXIT=0 at the standing 90. ⭐ **The calibration finding**: Unlighthouse's CI default applied **mobile 4G network throttle (150 ms RTT / 1.6 Mbps) to a DESKTOP form factor** — a hybrid no preset defines — scoring this site **0.78** where gate-19's fixture instrument (`--preset=desktop`, 40 ms / 10.24 Mbps) reads **0.95–1.0**; *a bar transcribed from one instrument and enforced by a stricter hybrid is two instruments sharing one number.* The sweep is now **pinned to the bar's own instrument**, measured not assumed (`unlighthouse.config.ts`, throttling block named + dated). **AC3 ✅.** Bars stay transcribed-from-gate-19, **un-sourced (F-e) — provenance is B2b's, ◐ OWED**. ⛩ Owed: the **first on-GitHub scheduled run** (unblock = the next ⛩ push GO carries the workflow). | sweep + enforced lane | **— none.** ⭐ *This is the whole point of the split: B2a never had an external dependency and was blocked only by being bundled with B2b.* |
| ✅ **B2b** | ~~Budget config; **provenance tested by hash**, not asserted (AC4 as replaced)~~ **DONE 2026-09-02.** `bar_provenance.json` + **`gate-53`** (G53a–G53f), red-proven **9/9** (7 mutations + 2 controls), **every case red at its DECLARED assertion set** — GR-3's attribution clause applied from the harness's first run rather than discovered in its fourteenth day. ⭐ **The hash is over `classes.content_static`, NOT the file — and that choice is MEASURED**: between our pin `6096157` and their HEAD `b7c6d653` the *file* moved **43,988 → 52,879 B across 5 commits** while the *class subtree* did **not move at all** (sha256 `f2292ca9…` at both). A file hash would have redded all five for reasons unrelated to our bars. ⭐ **G53e asserts the EXCLUSIONS** (gate-48's discipline): every leaf of the class is mapped or dispositioned, so a metric WebForge adds reds this gate until someone rules on it — *silence is not a disposition*. ⚠ Writing that assertion surfaced a leaf (`/categories/performance`) covered by neither, **declined in prose only** — the rule earned itself before it ran. | budgets + hash gate | ✅ **CLEARED — ⊳ D-E ANSWERED (scope B).** ~~⛔ **HELD — ⛩ ruled (c) 2026-08-26.**~~ ⊳ D-E (Vitruvius): **DELIVERED 2026-08-27** (`44c4d79`, P4.5b O4; `ack_required: true`); ~~**no reply yet** `[D]`~~ ⭐⭐ **ANSWERED — and the reply had been sitting UNTRACKED IN OUR OWN INBOX since 2026-08-29 14:20** (`coord_2026_08_26_vitruvius_to_rosetta_scope_b_and_your_three_findings_verified.md`, committed at `d2228b0`). It rules **SCOPE B**: *"`adna.network` does not enter the fleet roster… A static Astro build with no SSR app shell is `content_static` — that is what the class is. **Your reading is right, so name it and derive against it.**"* ⚠ **This row said *"no reply yet `[D]`"* for four days after the bytes arrived**, and so did two mission closes — the hold was verified in **Vitruvius's 08-27 prose notice**, never at **the destination the claim was about**. ⭐ babbage's 08-29 memo **named the three untracked memos explicitly** and *that* memo was untracked too. *`F-u`'s class, fourth sighting; convention 19's finding at the level of the inbox.* — surface named: `who/coordination/` here **and** `WebForge.aDNA/who/coordination/`. B2b's *shape* depends on their answer (fleet surface = scope A · derived-against-a-named-class = scope B). **If a reply lands mid-increment, B2b re-enters at ITS OWN ⛩ gate — it does not silently absorb into B2a.** |
| ✅ **B3** | ~~Close `idea_visual_regression_gate` (resolved) + runbook updates + AAR~~ **DONE 2026-09-02 — and P4.4 closes with it.** `idea_visual_regression_gate` → **`resolved`**, read **clause by clause** rather than ticked: three of four acceptance clauses met, and the fourth — *"wired into `test:gates`"* — ⛔ **deliberately refused**, because `test:gates` was bare `playwright test` and honouring that clause literally would have dragged the snapshot lane onto macOS for **24 red on a good tree**: *precisely the "false-positive churn that trains reviewers to ignore the gate" this idea was deferred to avoid.* ⭐ **The clause was written before anyone knew the lane would be container-only, so obeying it would have produced the exact failure it was written to prevent** — recorded as a deviation, not ticked. ⭐⭐ **Runbook finding: `skill_web_quality_sweep`'s Step-2 baseline MIXED DENOMINATORS** — *"633 → 659 at B0"* put a **chromium** figure and an **all-projects** figure on the two ends of one arrow, while the step prescribes `npm run test:gates`, which is chromium only. **And chromium today genuinely IS 659, so the stale line read as correct for the wrong reason** — the hardest kind of stale number to catch, and *the second instance in this increment of a coincidental match looking like a passed check* (the first being LCP/CLS equalling `content_static`). Replaced with a per-command table, each number naming the command that prints it: chromium **659** · fast **541** · snapshot **26** · all-projects **685** = 659 + 26, all derived `[D] 2026-09-02`. **AAR filed** (SO#5, file end). | records + AAR | — |
...
> than re-run.
>
> ⛩ **AMENDMENT 2026-08-28 (GR-1 O2 — AC-2 / GAP-1): `F-w` ADDED.** The Grande Revue's **P2-3**
> (the vendored marketplace/agentic-residuals promise) enters the register with a named destination —
> **the next `skill_template_release`, joining F-k** — rather than living in prose. **Re-derived at this
> commit, not carried: total 21 · struck 15 · live 6** (F-d · F-e · F-j · F-k · F-v · **F-w**).
> ⭐ It is a row because GR-1's convention-13 pass found AC-2's memo half **tested by nothing** (GAP-1) —
> P4.3's F-v lesson applied the moment it recurred, rather than after.
>
> ⛩ **AMENDMENT 2026-08-28 (GRANDE REVUE Gate 1, Lane B — P1-8): `F-n` STRUCK.** Discharged in fact
> by `6675442` on 08-25 (the MANIFEST content review, ratchet 49 → 0); left live on paper for three
> days — *the debt ledger carrying the class it exists to fence*, and the third time this register has
> been wrong by being read rather than re-run. **Re-derived at this commit, not carried:** total
> **20** · struck **14** · live **6**.
>
> ⛩ **AMENDMENT 2026-08-28, same sitting (Lane B5 — P1-6): `F-m` STRUCK — and this time the strike
> rides the fixing commit**, which is the P1-8 lesson applied one hour after learning it. The adr_index
> backfill (047–059, tally 41 → 54, drift-check glob corrected) is the fix F-m's own note named as
> unbuilt. **Re-derived at this commit, not carried:** total **20** · struck **15** · live **5**
> (F-d · F-e · F-j · F-k · F-v).
>
> ⛩⛩ **AMENDMENT 2026-09-01 (`GR-2` O5): `F-x` AUTHORED AND STRUCK IN THE SAME COMMIT, AND `F-y`
> ADDED.** **Re-derived at this commit, not carried:** total **23** · struck **16** · live **7**
> (F-d · F-e · F-j · F-k · F-v · F-w · **F-y**).
>
> ⭐⭐ **THE ROW HAD TO BE WRITTEN BEFORE IT COULD BE STRUCK, AND THAT IS THE ENTRY WORTH READING.**
> `GR-2` was convened on `F-x` and its final objective was *"strike `F-x`"* — but the register held no
> such row. The 08-29 campaign `CLAUDE.md` says *"**`F-x` added** … Register **22 total · 15 struck ·
> 7 live**"*; the register said **21 / 15 / 6** `[D]`. ⇒ **the routing claim was never true at its
> destination and the tally that counted the missing row was typed** (KW-14). This is **`F-u`'s exact
> class, third sighting** — and note where it landed: on the one row an entire mission existed to
> discharge. *A mission can be convened on a debt that was never recorded.*
>
> ⚠ **The gap is not one row wide.** The `F-` letter sequence reads **a–r, u, v, w**: `t` is missing
> for a documented reason (withdrawn as a duplicate of `F-l`), and **`s` and `x` are missing because
> nobody wrote the row.** `F-s` — the live production regression that raised the deploy freeze and cost
> four missions of unshipped work — is cited by ID throughout this campaign and **still has no row**.
> That is `F-y`, which states the habit — **an `F-` ID is minted by writing the row** — and, per
> conventions 15/16/17, **builds no checker for it**. ⛔ `F-s`'s backfill is named, not smuggled into
> `GR-2`'s scope.
>
...
| **F-e** | ~~**⊳ D-E — mirror `lighthouse_profiles.json`** into `how/federation/webforge/`, or amend campaign convention 4~~ **→ THE ASK IS AUTHORED AND STAGED; THE MIRROR IS WITHDRAWN AS THE WRONG MECHANISM** | ⛩ DP6 | ~~`find . -name lighthouse_profiles.json` → **0 hits** vault-wide, so every gate-19 bar is currently a transcription~~ **— three of this row's four load-bearing claims are FALSE, measured 2026-08-24** | ✅✅ **STRUCK 2026-09-02 AT P4.4b B2b — THE BARS ARE SOURCED.** `site/tests/gates/bar_provenance.json` records **per-bar** provenance and `gate-53` enforces it (6 assertions, red-proven **9/9** — 7 mutations + 2 controls, every case red at its **declared** assertion set per GR-3's attribution clause). ⭐⭐ **And the row's own remaining premise did not survive the object either.** This row, and every citation of it, said our **Perf ≥ 90 is LOOSER than `content_static`'s 95 — "the direction their `ratchet_law` reserves for an operator gate."** `_meta.lh_settings` `[D]`: *"Lighthouse default config: **mobile emulation** + simulated throttling… **Desktop pass: not wired**."* ⇒ **their 95 is a MOBILE bar, `gate-19`'s 90 is a DESKTOP bar, and no desktop `content_static` bar exists to be looser than.** B2a had already measured the size of that difference — mobile throttling on a desktop form factor scored this site **0.78** against the desktop instrument's **0.95–1.0**: the **form-factor gap is ~17 points and the bar gap is 5**. ⇒ *not a relaxation — a **category error**, and `ratchet_law` never bound us because we never adopted their bar.* ⛩ **Ruled 2026-09-02: HOLD 0.9 and SOURCE it** ([[b2b_reentry_proposal]], `accepted`) — so **F-e closes by recording where each number came from, not by moving one**, and `gate-51`'s drift detector stays green with no same-diff churn. ⭐ **Two of our bars equal theirs (LCP 2500 · CLS 0.1) and neither was derived from theirs** — both sides took the CWV *Good* band independently. **EQUALITY IS NOT PROVENANCE**, and recording them as "adopted" would have asserted a derivation that never happened. ⚠ **Residual, named not smuggled**: `gate-19`'s four fixtures carry **`configSettings` ABSENT**, so *"our 90 is a desktop bar"* is **`[I]` at the artifact, not `[D]`** — B2b can source the **bar**, it cannot retroactively attribute the **fixtures**; and four leaves of the class (**TBT 200** + a11y/best-practices/seo) are ⛩ ruled **un-adopted with reasons**, asserted by **G53e**, never silent. ~~⛩ **ASK STAGED at A3, 2026-08-24; ROW STAYS LIVE** (staged ≠ delivered — live count holds at **6**).~~ ⚠ **And the reason it stayed live nine extra days is its own finding**: Vitruvius's scope-B answer was **delivered on 2026-08-29** and sat **untracked in `who/coordination/`**, while this row and two mission closes recorded it as *"no reply yet `[D]`"* — the block verified in the peer's *prose notice*, never in **our own inbox**. *`F-u`'s class, fourth sighting.* ⭐ **The row's premise did not survive the object.** (1) **The file exists** — `WebForge.aDNA/what/lib/gates/lighthouse_profiles.json`, 43,988 B. The `find` ran over **our** vault and its output said *"0 hits vault-wide"* without naming the surface it searched — breaching **convention 16** (*a negative result is only as wide as the command that produced it*) and **convention 17** (*every absence assertion names its surface*), both authored by this campaign. (2) **Our pin is byte-current against it**, hash-verified: md5 `134c9647c4c348034db3fa32d65d9db1` identical at pin `6096157`, at their HEAD `14838774`, and in their working tree; `git log 6096157..HEAD -- <path>` empty. (3) **Nothing of ours reads it** — `grep -rn lighthouse_profiles site/` → **0**. `gate-19`'s bars (LCP 2500 · CLS 0.1 · **Perf ≥ 90**) are the CWV *Good band* asserted over committed **slim desktop** fixtures (LH **13.4.0**, `configSettings` absent); their `content_static` is Perf **95** · a11y 95 · bp 95 · seo 100 · **tbt 200**, measured live under mobile emulation + simulated throttling, N=3, LH **13.4.1**, bound by `rebaseline_law` to a host **and browser** fingerprint we do not match. ⇒ the bars are **UN-SOURCED, not transcribed**, and a mirror would not have made them derived — while **Perf 90 is LOOSER than the class 95**, the direction their `ratchet_law` reserves for an operator gate. (4) **The mirror is contradicted from both ends**: our wrapper's line 24 names **gates** among what is *"consumed by reference, never copied"*, and their own `CLAUDE.md` says the bars *"are class-keyed data … **read them there and never transcribe them**"*. ⭐⭐ **AND THE RE-READ DISCIPLINE DID NOT CATCH IT AT A1 — IT RE-RAN THE SAME UNDER-SCOPED COMMAND AND CONFIRMED THE WRONG THING.** *(Struck above.)* **Re-reading a row at the object fails when you re-run the row's instrument instead of re-deriving its question** — the F-u class (a remedy fixed at the moment of diagnosis) meeting the convention-16 class (an instrument narrower than its conclusion). ⇒ **Ruling 3 stands and is honoured**: the ask ships and convention 4 is **not** amended by us — only its *evidence sentence* is corrected to name its surface. Memo: [[coord_2026_08_24_rosetta_to_vitruvius_profiles_are_read_not_mirrored]], `status: staged` — ⛩ delivery is a separate outward act. ⚠ **Residual, and it is the real debt**: our bars are still un-sourced, which **⛔ P4.4b's AC4 does not close by hashing a mirror** — see AC4's row.
...
| **F-w** | **The public mirror of `skill_onboarding` promises a "Lattice Protocol **marketplace** … with **agentic residuals** flowing back to you", hedged only by "coming soon"** — an unbacked forward ECONOMIC claim, live on `/get-started/what-your-agent-reads/skill-onboarding/`, in its `.md` twin, and at `dist/llms-full.txt:2455`. | GRANDE REVUE **P2-3** (2026-08-28) | ⛩ **ROUTED → the next `skill_template_release`, joining F-k** (operator-opened; no date pinned, deliberately). ⭐⭐ **NOT FIXABLE HERE, AND THE REASON IS THE POINT.** The file is **byte-vendored** from `.adna` by `site/scripts/build_tour_files.mjs`, and `/get-started/what-your-agent-reads/` **publishes its sha256 and invites the reader to diff it against their own clone**. Editing the site copy would make that page's central promise false — **trading a copy defect for a trust defect on the one surface built to be checked**. Standing Rule 1 forbids touching `.adna/` directly, so the release is the only sanctioned path. ⛩ **GR-1 ruled 2026-08-28: site-side display only; no vendored-byte edit.** ⚠ **A note for whoever fires that release:** GR-1's O4 re-pinned the tour to the **upstream release tag** (`source_ref`), so the fix reaches the site the moment the next release ships and the version bumps — no separate site change is owed, and none should be invented. ⭐ Filed as a ROW rather than a sentence because AC-2's own pass (GAP-1) found the memo half was **covered by no verification limb**; `gate-17` G19's clause now asserts this row exists. |
| ~~**F-x**~~ | ✅ **DISCHARGED 2026-09-01 IN BOTH HALVES BY `GR-2` — AND THE ROW HAD TO BE AUTHORED BEFORE IT COULD BE STRUCK, WHICH IS THE FINDING.** ~~**`gate-33-freshness` is red in CI and the failure is TWO debts.** (a) The cause is unknown. (b) **The gate's error message names a remedy already applied** — it said *"set `fetch-depth: 0`"* while `gates.yml:51` had been `fetch-depth: 0` the entire time, so the gate converted a symptom into a confident wrong diagnosis: this campaign's own class, in its own CI.~~ | GRANDE REVUE deploy sitting (2026-08-29); **row authored at `GR-2` O5, 2026-09-01** | ⛔⛔ **THE ROW DID NOT EXIST UNTIL THE MOMENT IT WAS STRUCK.** The 08-29 campaign `CLAUDE.md` says *"**`F-x` added** … Register **22 total · 15 struck · 7 live**"*; the register said **21 / 15 / 6** and held no `F-x` row `[D]`. ⇒ **the routing claim was never true at its destination, and the tally that counted the row was TYPED** (KW-14). ⭐ **This is `F-u`'s exact class, third sighting** — *"'Routed' is a claim about the destination, so verify it there, never in the prose that routed it"* — and it landed on the one row a mission was convened to discharge: **`GR-2` O5 set out to strike a row that did not exist.** The row is written here in full, then struck, rather than discharged silently, so the ledger shows what was owed and not merely that nothing is owed now (SO#6). **Both halves discharged:** (a) the cause is named **at the object, from CI, in git's own words** — run `33465663585`, `fatal: detected dubious ownership in repository at '/__w/aDNA.aDNA/aDNA.aDNA'`; **not** a shallow clone, so `fetch-depth` was never the lever (`artifacts/gr_2/o3_ci_reason_record.md`). (b) The message was fixed at O2 — `contentSource.ts` carries three states, `git()` pipes stderr instead of discarding it, and the gate points at the build's own diagnostic instead of prescribing a cure (`gate-52`, red-proven 8/8). **Remedied and verified at the run**: one `gates.yml` step, `safe.directory` scoped to `$GITHUB_WORKSPACE` and never `*`, run `33467130677` **success** against `33465663585` as the red control (`artifacts/gr_2/o4_ci_green_record.md`). ⚠ **And the streak this row cites was understated three times** — six → seven → **never green**: the assertion landed at `0eb48fa` *after* the last green run and **no `gates` run on `main` succeeded between them**, so it had never once passed in CI. ⛔ **No checker is authored for the class** (conventions 15/16/17): what would have caught this is reading the destination, and `F-y` records it as a habit. |
| **F-y** | **THE REGISTER'S TALLY IS CARRIED ARITHMETIC, AND THE `F-` LETTER SEQUENCE HAS HOLES WHERE FINDINGS WERE NAMED IN PROSE AND NEVER GIVEN A ROW.** Derived `[D]` 2026-09-01: `grep -oE '^\| ~?~?\*\*F-[a-z]\*\*'` yields the letters **a–r, u, v, w** — **`s` and `x` are missing**, and `t` is missing for a *documented* reason (withdrawn as a duplicate of `F-l`). **`F-s`** — the live production regression of 2026-08-23, which raised the deploy freeze and cost four missions of unshipped work — is cited by ID across this campaign and **has no row**. **`F-x`** was the same, until the row above was written. ⇒ **no single command can derive this register's total**, because an ID minted in prose does not reach the table, and the totals in the campaign `CLAUDE.md` are therefore *arithmetic carried forward* rather than a reading. | `GR-2` O5 (2026-09-01), sharpened from the O3 handoff's *"the register has no derivable tally"* | ⛩ **STATED AS A HABIT, NOT BUILT AS A CHECKER** — conventions 15/16/17 all ruled this shape, and the standing count of this desk's instruments found defective before their subjects is the argument. **The habit: an `F-` ID is minted BY WRITING THE ROW.** Naming one in a session record, a commit message or the campaign `CLAUDE.md` does not create it; the row is the mint. And **any tally quoted anywhere is re-derived by the three greps at the commit that quotes it** — never carried, since this register is the campaign's own worked example of *an instrument whose output changes its own input* (`F-q`'s class) and has now been wrong by being read rather than re-run **four** times. ⚠ **Not discharged by this row**: `F-s` still has no row. Writing it is a backfill of a *closed* incident and is deliberately **not** smuggled into `GR-2`'s scope — it is named here so the gap is a debt with a home instead of a silence. ⭐ Filed as a ROW rather than a sentence for the reason the row above exists: **that is the defect.** |
| ~~**F-s**~~ | ✅ **BACKFILLED AND STRUCK 2026-09-01 BY `GR-3` O5b, ⛩ OPERATOR-RULED IN — a row for a CLOSED incident, written because the silence was the defect.** ~~**Production served a build predating 2026-08-18 for an unknown interval**: `/api/registry.v1.json` and `/vaults.json` 404, `/state-of-the-network` 404 in all three forms while the footer linked it, P2.1/P2.2 redirects un-fired, the P3.3 machine-door block absent. Ten prod deploys had landed that day from an actor outside this vault's deploy discipline, none in any `deploy_log*` fleet-wide.~~ | raised 2026-08-23 (`artifacts/p4_1/finding_live_prod_regression_20260823.md`); **row authored at `GR-3` O5b, 2026-09-01** | ⛔⛔ **THIS ROW DID NOT EXIST FOR NINE DAYS WHILE THE FINDING IT NAMES WAS CITED BY ID ACROSS THE CAMPAIGN** — in convention 16, in the freeze, in three missions' prose. `F-y` registered the gap; `GR-2` explicitly declined to smuggle the backfill into its own scope; ⛩ the operator ruled it into `GR-3` at the pre-build gate. ⭐ **Its substance was discharged long ago and by other hands**: cause found 2026-08-24 (the lemur checkout of this same repo, deploying its own local commits), freeze lifted 2026-08-25 on the ancestry guard (`4a9bc09`, red-proven 13/13), and prod today serves `a852423` — **an ancestor of `main`**, verified at the open. What was owed was never the fix. **It was the row.** ⇒ the debt discharged here is a *bookkeeping* one, and it is struck in the same commit it is written, exactly as `F-x` was — so the ledger shows what was owed rather than merely that nothing is owed now (SO#6). |
| ~~**F-z**~~ | ✅ **DISCHARGED 2026-09-01 IN BOTH HALVES BY `GR-3`.** ~~**(a) `gate-42` fails the site for requests `gate-42` itself cancelled.** Its traversal used `waitUntil: 'load'` + `waitForTimeout(120)` — a bet that a page's post-`load` lazy work finishes in 120 ms. `MermaidDiagram.astro` does `await import('mermaid')`, so on a loaded CI runner the next `goto` cancels the in-flight chunks and the `net::ERR_ABORTED`s are reported as the SITE's broken same-origin assets. Failed `main` at `2ad7768` (run `33558250778`, 6 aborts on `/learn/concepts/context-commons/`, light mode only) on a commit where **nothing but prose had changed**. **(b) The `assetFailures` assertion had NEVER ONCE been demonstrated to fail.** `console_clean_redtest.sh` case 3 was believed to red-prove it and does not: a deleted file is a 404 RESPONSE, `hits` is asserted first, a non-soft `expect` throws immediately, so `assetFailures` was never evaluated.~~ | `GR-3` (2026-09-01), found by **convention 19** at session open — the habit `GR-2` had adopted the day before | ⭐⭐ **THE CAMPAIGN'S FIRST FALSE *RED*.** Conventions 14 and 16 were both built from false *greens*; this is the mirror, and it is worse in one specific way — the gate's own message argues that gating on things that are not ours would be *"training people to ignore a red build"*, which is exactly what a flaky assertion does, **one session after `GR-2` spent a mission on the finding that nobody read a red for fourteen days.** ⭐ **Non-determinism DEMONSTRATED, not argued**: `gh run rerun 33558250778` on byte-identical input came back **success** — same commit, opposite outcome. ⭐⭐ **Half (b) is convention 14's MISSING SECOND CLAUSE, and it outlives this fix: _a demonstration is only worth what it can attribute._** A red that cannot be traced to the assertion under test proves the gate is alive, not that the assertion is. Remedied: traversal drains to zero in-flight requests with expiry ASSERTED (`unsettled`, placed BEFORE the same-origin check so the cause reports ahead of the symptom); all three predicates byte-unchanged; harness now matches assertion identity on every case and a wrong-assertion red reports as a HARNESS BUG. Red-proof **8 pass / 0 fail (6 mutations + 2 controls)**, case 6 red **before AND after** the fix. ⚠ **The settle guard is deliberately NOT red-proven** — a race has no deterministic mutation, and conventions 15/16 forbid the sixth tail-authored instrument. ⚠ **Bound, not removal**: a page that goes quiet and then fires a delayed request can still be raced; `networkidle` carries the identical residual. Records: `artifacts/gr_3/`. |
...
> **Scope: P4.4b only** (B0 · B1 · B2a · B2b · B3). P4.4a's AAR is above, filed 2026-08-24, and stands
> unchanged. Filed 2026-09-02 at B3, session `session_stanley_20260902_005545_haussmann_p4_4b_b2b`.
> **Criteria: AC1 ✅ AC2 ✅ AC3 ✅ AC4 ✅ · V1 ✅ V2 ✅ V3 ✅ V4 ✅.** With P4.4a's AC0 ✅ + V5 ✅, **every
> criterion of P4.4 is met** and the mission closes.

**Worked** — **the pre-build gate, for the tenth consecutive mission, and this time it changed what
got built rather than what got budgeted.** B2b's re-entry pass read AC4 against V2 in both directions
and found that the increment's *entire premise* — *"our 90 is looser than their 95"* — compared two
numbers produced by different instruments on different form factors. Had B2b been built to its
ratified description it would have transcribed a **mobile** bar onto a **desktop** instrument, which
is the defect B2a had fixed eight days earlier, run backwards. ⭐ **The pass did not refine the plan;
it falsified it, before a line was written.**

**Didn't** — **B2b sat blocked for nine days on an answer that had been on this disk for four of
them.** The scope-B reply landed 2026-08-29; this mission's own B2b row, GR-2's close and GR-3's
close each recorded *"no reply yet `[D]`"* afterwards. The hold was re-verified in **the peer's prose
notice**, never in **our own `who/coordination/`** — the destination the claim was about. ⚠ And
babbage's 08-29 memo **named the untracked memos explicitly**, so the warning existed, in the inbox,
unread, for the same reason as the thing it was warning about.

**Finding** — ⭐⭐ **EQUALITY IS NOT PROVENANCE.** Two of gate-19's three bars (LCP 2500, CLS 0.1) are
*identical* to `content_static`'s and were **never derived from it** — both sides independently took
the CWV *Good* band. For three weeks the campaign read that identity as evidence of a relationship
that did not exist, and read the one *non*-identical bar as a policy violation (`ratchet_law`) when
it was a **category error**. ⇒ *A number that matches its supposed source is the hardest kind of
un-sourced number to notice, because it looks like the check already passed.* This is what closing
`F-e` actually consisted of: **not changing a bar, but recording where each one came from** — and the
record now distinguishes *derived-from*, *equal-by-common-ancestry*, and *no-counterpart-exists*,
which prose never could.

**Change** — **provenance is recorded PER BAR, and exclusions are asserted rather than narrated.**
`gate-53`'s **G53e** holds every leaf of the source class to a disposition: mapped, declined, or
un-adopted-with-a-reason. A metric WebForge *adds* now reds this gate until someone rules on it.
⭐ Writing that assertion immediately surfaced a leaf (`/categories/performance`) covered by
**neither** — declined in prose only — so the rule earned itself **before its first run**. Second
change: **hash the class subtree, never the file**, which is measured rather than argued — the file
moved 43,988 → 52,879 B across 5 commits while the subtree did not move at all.

**Follow-up** — three, each with a named destination and none smuggled into this close:
(1) **`gate-19`'s fixtures carry `configSettings` ABSENT**, so *"our 90 is a desktop bar"* is `[I]`,
not `[D]`; only a re-baseline repairs it. (2) **AC2's owed-list stands** — ⛩ Speed Insights enable →
...
5 sessions against a ratified 3** (B0 · B1+B2a · B2b+B3, plus the two gate sittings), which is inside
the *token* band per increment but over on session count — the driver is that **two increments
acquired their own operator gates mid-mission** (the B2/B2a-B2b split, and B2b's re-entry), neither
of which existed when the 3 was set. *A session count costed before a split is a count of a different
mission.*

⚠ **The standing instrument-defect streak continued and was caught by structure, not vigilance:**
`gate-53` shipped with **two defects of mine before it ran** — a JS canonicaliser that would not have
reproduced the Python hash (irreproducible provenance that *looked* correct), and a vacuous final
assertion in G53a. Both found by asking what each line actually proves. ⭐ And **G53f's live limb was
verified by its red-test, not by its green run**: two mutation cases turn it red, which is what
