...
instances:
  ...
  - "campaign_champollion.md M5.2 row — aDNA.aDNA (this vault; taught at '47935b6', pin-FOLLOWED to 8cb6e1e at M5.2 2026-07-02: empty spec diff, zero teaching change)"
...
- [[../../how/campaigns/campaign_champollion/campaign_champollion|campaign_champollion]] **M5.2 row** cites *"this vault's lattice/module/dataset teaching vs LP spec @ codepin `47935b6`"*. LatticeProtocol advanced their pin `47935b6` → `8cb6e1e` the same day (Carnot M1.5) — so as recorded, **this vault teaches at `47935b6` while upstream sits at `8cb6e1e`.** That lag is the mechanism working: pin-follow is a deliberate act this vault has not yet taken, and the gap is visible in one grep. **The follow happened at M5.2 (2026-07-02)** — the bilateral conformance check verified spec@`47935b6` == spec@`8cb6e1e` (empty `.agentic/specs/` diff, [[../../how/campaigns/campaign_champollion/artifacts/conformance_bilateral|conformance_bilateral]] §1/§4) and the fable review ratified the follow: the recorded teaching pin is now **`8cb6e1e`**, matching LP's `codepin.md`, at zero teaching change. The lag-then-deliberate-follow arc above stands verbatim as the pattern's worked example — a full cycle of the mechanism (drift visible → re-verify → follow) completed in the open.
