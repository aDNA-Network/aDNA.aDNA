...
| ~~`LatticeLabs.aDNA/`~~ | Berthier | Lattice Protocol operation (predecessor; at-current-standard successor to `lattice-labs/`) | — | Lattice Council (inherited → aDNALabs) | **MERGED → `aDNALabs.aDNA` (PT pt11, 2026-06-17)** — predecessor consolidated into the HQ; 2 live campaigns (`campaign_latticeprotocol_extraction` + `campaign_latticelabs_genesis`) relocated; source archived → `Archive.aDNA/LatticeLabs.aDNA/`. `lattice-labs/` + war/federation stay until W-NET (P5.5/Network P6). |
