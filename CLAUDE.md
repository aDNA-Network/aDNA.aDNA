...
| `skill_node_bootstrap_interview` | agent | **18**-question operator-specific node-vault bootstrap interview (~4–7 min) — ⛩ `19 → 18` at the v8.11 gate, 2026-09-11, when `C4` (marketplace categories) was struck as `F-w`'s family. ⚠ **The workspace router (`~/aDNA/CLAUDE.md`) still says 19 and is CORRECT until the release lands**: it describes what a *fork* runs, which is `.adna/`, and `.adna/` still asks 19. **This row describes our disk; that row describes the image.** Two surfaces, two objects — and they are allowed to disagree for exactly as long as the gate is open, provided somebody says which is which. This sentence is that somebody. The router is `Home.aDNA`'s file via symlink and moves by memo, never by an edit from here (Rule 10). |
