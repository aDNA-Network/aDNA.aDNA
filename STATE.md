...
**As of 2026-05-19 (v8 P2 — M2.1 S2 destructive in flight; Op 1 STATE.md split phase A landed; Op 1 phase B router rewrite in flight this commit; Op 2 + S3 close pending)**:
...
- **OPENED**: `campaign_adna_serious_tool_readiness` (v8) M2.1 Context File Audit + STATE.md Split **S1** at 2026-05-19T19:33:44Z+ (`session_stanley_20260519T193344Z_v8_m21_s1`; plan ratified at `/Users/stanley/.claude/plans/please-read-the-claude-md-fluffy-stream.md`). **P1 → P2 phase exit gate APPROVED at plan ratification** per Campaign S.O. #19 + project S.O. #1; M1.5 coord-network stays operator-discretionary parallel (NOT blocking on P2 transition). **4 of 7 M2.1 deliverables landed this S1** (mission_class implementation; canonical 3-session shape; 3rd instance after M1.3 + M1.4): (1) `missions/mission_adna_str_p2_m21_context_audit_split.md` — mission spec (7 objectives S1(1-4)/S2(5-6)/S3(7); 11+ acceptance; D1-D5 all defaults A resolved at plan; 13-row S.O. discharge; 7 hard constraints; token_budget S1 60-90 / S2 100-180 / S3 60-80 kT); (2) `missions/artifacts/m21_obj2_state_split_design.md` — Op 1 STATE.md split design (pre-split baseline 347 KB / ~75.8 kT across 3 zones; router preserves lines 1-333 trimmed; archive carries 19 DEPRECATED-marker blocks + 1 superseded `Last Session` + historic Next Session Prompts verbatim; 4-phase S2 commit sequence; <1 min rollback; target router ≤ 20 kT / ≥ 4× reduction); (3) `missions/artifacts/m21_obj3_agents_md_hint_design.md` — Op 2 Heavy-File Read Convention (≤ 5-line hint at `aDNA.aDNA/AGENTS.md`; companion `MEMORY.md` index + memory file per two-step memory protocol; upstream backlog placeholder at `how/backlog/idea_upstream_state_md_read_hint.md` for v8 P6 propagation); (4) `missions/artifacts/m21_obj4_archive_convention_design.md` — Op 3 auto-archive convention as **doctrine-as-design-memo, NOT ADR text** (Campaign S.O. #14); STATE.md as canonical first instance; skill graduation `skill_campaign_close_archive.md` deferred to after 3 distinct rotations; retroactive `campaign_*_archive.md` for v2 + LWX masters deferred to M3.x per D4=A. **Load-bearing empirical signal**: Read tool on pre-split STATE.md from this very session returned `File content (339.2KB) exceeds maximum allowed size (256KB)` — confirms the M1.4 Mid-magnitude (refined) verdict's API-billing companion thesis and the urgency of Op 1. **Hard constraints honored**: zero `.adna/` touches (v7.0 frozen at `27e6395`); zero partner-vault contact (4 embargo memos preserved); zero `~/.claude/settings.json` modifications; zero ADR drafts at M2.1 S1 (ADR-016 ratifies at M2.2); zero STATE.md destructive structural changes (S2 owns the split); zero `node.aDNA/` mutations; zero M1.5 work. **S2 destructive entry (Op 1 STATE.md split + Op 2 AGENTS.md/MEMORY.md/backlog writes) operator-gated** per Standing Order #1; 4-phase S2 commit sequence specified. **Next**: M2.1 S2 opens at operator discretion (non-blocking; design artifacts mechanical to execute) → M2.1 S3 close → M2.2 ADR-016 ratification (or any M2.x parallel entry per Campaign S.O. #19). M1.5 coord-network discharge stays operator-discretionary parallel.
...
**As of 2026-05-19 (v8 P2 — M2.1 in flight; S2 destructive opened this session) — Primary track: `campaign_adna_serious_tool_readiness`**

### v8 P2 status + parallel-discretionary slate

1. **M2.1 Context File Audit + STATE.md Split** — `status: in_progress`; S1 closed 2026-05-19T19:33Z (4/7 deliverables: mission spec + Op 1/2/3 designs); **S2 destructive in flight** (this session; Op 1 STATE.md split phase A landed at SHA `95332df`; phase B router rewrite this commit; Op 2 AGENTS.md/MEMORY/memory file/backlog pending; S3 close pending). See [[how/campaigns/campaign_adna_serious_tool_readiness/missions/mission_adna_str_p2_m21_context_audit_split.md|M2.1 mission file]].

2. **M1.5 coord-network discharge** — `status: planned`; operator-discretionary parallel slot. Timing target ≤ 2026-05-26 per [[who/coordination/coord_2026_05_19_v8_cross_vault_disruption_assessment.md|cross-vault disruption assessment memo]] §6. Discharges LIP-0006 ratification + 2 entity-types parallel-discharge + 5 open Qs at [[who/coordination/coord_2026_05_19_v8_cross_vault_network_coordination.md|M1.5 coord memo]]. 0-1 sessions; non-destructive.

3. **M2.2 ADR-016 ratification** — `status: planned`; operator-discretionary parallel slot. Consumes M1.3 §6 + M1.4 §6 verbatim prep notes. Pure governance; non-destructive.

4. **M2.3 Convergence-model validation cross-campaign retrospective** — `status: planned`; operator-discretionary parallel slot. Consumes `ingest_transcript.py` corpus (645 MT cache_read across 48 sessions) + API-billing companion formula forecast.

5. **M2.4 AGENTS.md heat map** — `status: planned`; awaits ≥ 10 live sessions of corpus accumulation. Current corpus = 48 sessions but pre-hook majority; live-hook corpus ~2 sessions. Defer to late P2.

### Operator pre-S3 review options

- Review M2.1 S2 split outputs (this commit + Op 2 commit pending): byte-count router/archive + AGENTS.md hint live + memory feedback file + backlog placeholder.
- Spot-walk `node.aDNA/what/context/token_baselines.md` v0.1.1 baseline before S3 light addendum.
- Authorize M1.5 entry now (parallel; not blocking M2.1 S3 close).
- Authorize M2.2 ADR-016 ratification now (parallel; pure governance).
- Pause to validate router shape with a fresh cold-start session (Read STATE.md no-offset succeeds; cache_creation tax measurably lower).
