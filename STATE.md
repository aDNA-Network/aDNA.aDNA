...
status: active
phase: "genesis_planning"                            # Operation HAUSSMANN genesis session executing (directive 2026-08-16); was "production" steady-state since Palimpsest close 07-24; P<n>[/<count>] grammar — see what/doctrine/doctrine_state_conventions.md §2
campaigns: []                                        # active campaign ids (honest-absent → []); campaign_haussmann is at GENESIS (status: planning) — registers here only at Gate C ratification (§7.7)
