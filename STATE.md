...
type: state
created: 2026-04-13
updated: 2026-07-24  # 2026-07-24 ✅ REGISTRY REGEN 73→74 (RareAnthropic.aDNA projected into `vaults.json`; Home `faf5eac` local/no-push → 74/14 edges sha `536e9d62`, org_graph 3→4 → G20 fixture 73→74 → 203pp → 371/371 gates → `498f985`). **Full inline PRIOR-chain archive-shifted verbatim → [[STATE_archive]] §Shifted-2026-08-03** (second graduation of this key; the first, ~29.7KB, went to §Shifted-2026-07-17).
status: active
phase: "production"                                  # steady state (no active campaign; Palimpsest closed 2026-07-24); P<n>[/<count>] grammar — see what/doctrine/doctrine_state_conventions.md §2
campaigns: []                                        # active campaign ids (honest-absent → []); v8.7 sibling key — mission: omitted (honest-absent; no mission of record between campaigns)
