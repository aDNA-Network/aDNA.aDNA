...
# One-run contract (2026-09-01): after the request is submitted, the user runs ONE more
# command -- immediately, before approval -- and is done; the background finisher does the
# rest. This block is the only "what next" a waiting enrollee should ever see.
function Show-FinishNextStep {
    Write-Host ""
    Say "One more command finishes everything. Run it NOW, in an ELEVATED PowerShell"
    Say "(right-click PowerShell -> Run as administrator) -- no need to wait for approval:"
    Write-Host ""
    Write-Host "  & ([scriptblock]::Create((irm $Base/install.ps1))) -Activate"
    Write-Host ""
    Say "After that, this machine joins the network automatically the moment your"
    Say "request is approved, and you are done. (Or rerun the same command later to check.)"
}

# -- the background finisher (one-run contract, 2026-09-01) ---------------------------------
# Approval is human-paced; every "rerun the installer later to check" was a user running the
# installer twice (it also SRV-05'd the first real Windows enrollee). -Activate run BEFORE
# the cert arrives now arms a SYSTEM scheduled task (every 5 minutes) running a generated,
# SELF-CONTAINED finish.ps1 with the endpoint, request id and CA fingerprint BAKED IN at arm
# time -- what runs privileged is auditable in one file and never a fresh download. On
# approval it delivers + pin-verifies + acks the cert, runs the activation gates, starts the
# service, and deletes its own task. Terminal states disarm loudly into finish.log.
function Arm-Finisher {
    param($Root, $PkiDir, $BinDir)
    $stateFile = Join-Path $Root 'enrollment_submission.json'
    if (-not (Test-Path $stateFile)) { $stateFile = Join-Path $Root 'uninvited_submission.json' }
    $state = Get-Content $stateFile -Raw | ConvertFrom-Json
    $task = 'aDNA-Finish-' + (Split-Path -Leaf $Root)
    $fin  = Join-Path $Root 'finish.ps1'
    $tpl = @'
# generated by the aDNA installer (-Activate) -- the background enrollment finisher.
# Runs as SYSTEM every 5 minutes; on approval: delivers + verifies the certificate against
# the CA fingerprint baked in below at arm time, confirms delivery, runs the activation
# gates, starts the service, and deletes its own scheduled task. Terminal states disarm
# loudly into finish.log -- a person needs to look.
$ErrorActionPreference = 'Stop'
function Note($m) { try { Add-Content -Path '__ROOT__\finish.log' -Value ("{0:yyyy-MM-ddTHH:mm:ssZ} {1}" -f (Get-Date).ToUniversalTime(), $m) } catch { }
}
function Disarm { & schtasks /Delete /TN '__TASK__' /F 2>$null | Out-Null }
try {
    $pki = '__PKI__'; $ca = Join-Path $pki 'ca.crt'; $crt = Join-Path $pki 'host.crt'
    if (-not ((Test-Path $ca) -and (Test-Path $crt))) {
        $st = Invoke-RestMethod -Uri '__ENDPOINT__/enroll/__RID__' -TimeoutSec 30
        if ($st.state -in @('refused', 'picked_up', 'delivered')) { Note ('terminal state ' + $st.state + ' -- DISARMED; a person needs to look'); Disarm; exit 0 }
        if ($st.state -ne 'ready') { exit 0 }
        $tmp = Join-Path $pki 'ca.crt.incoming'
        Set-Content -Path $tmp -Value $st.ca_crt -Encoding utf8
        $fps = & '__NEBULACERT__' print -json -path $tmp | ConvertFrom-Json
        if (@($fps).fingerprint -notcontains '__PINFP__') { Remove-Item $tmp -ErrorAction SilentlyContinue; Note 'received CA does not match the pinned fingerprint -- DISARMED; report this, do not retry'; Disarm; exit 1 }
        Move-Item $tmp $ca -Force
        Set-Content -Path $crt -Value $st.host_crt -Encoding utf8
        if ($st.ack_token) { try { Invoke-RestMethod -Method Post -Uri '__ENDPOINT__/enroll/__RID__/ack' -ContentType 'application/json' -Body (@{ ack_token = $st.ack_token } | ConvertTo-Json) | Out-Null } catch { } }
        Note 'certificates delivered, verified against the pinned CA, acked'
    }
    & '__NEBULACERT__' verify -ca $ca -crt $crt
    if ($LASTEXITCODE -ne 0) { Note 'certificate does not verify against the CA -- will not start the service'; exit 1 }
    $info = & '__NEBULACERT__' print -json -path $crt | ConvertFrom-Json
    if ($info -is [array]) { $info = $info[0] }
    $pubB64 = ((Get-Content (Join-Path $pki 'host.pub')) | Where-Object { $_ -notmatch '-----' }) -join ''
    $pubHex = ([BitConverter]::ToString([Convert]::FromBase64String($pubB64)) -replace '-', '').ToLower()
    if ($info.publicKey -and $info.publicKey -ne $pubHex) { Note 'CERTIFICATE / KEY MISMATCH -- DISARMED; ask for a certificate issued against the enrollment request from THIS machine'; Disarm; exit 1 }
    & '__NEBULA__' -test -config '__CFG__'
    if ($LASTEXITCODE -ne 0) { Note 'config does not load -- will retry next cycle'; exit 1 }
    $svc = Get-Service Nebula -ErrorAction SilentlyContinue
    if (-not $svc) {
        & '__NEBULA__' -service install -config '__CFG__'
        if ($LASTEXITCODE -ne 0) { Note 'service install failed -- will retry next cycle'; exit 1 }
    }
    & '__NEBULA__' -service start -config '__CFG__' 2>&1 | Out-Null
    Start-Sleep -Seconds 3
    $svc = Get-Service Nebula -ErrorAction SilentlyContinue
    if ($svc -and $svc.Status -eq 'Running') { Note 'service running -- JOINED; finisher removed itself'; Disarm }
    else { Note 'service not running yet -- will retry next cycle' }
} catch { Note ('cycle error: ' + $_.Exception.Message) }
'@
    $body = $tpl
    $body = $body.Replace('__ROOT__', $Root)
    $body = $body.Replace('__PKI__', $PkiDir)
    $body = $body.Replace('__ENDPOINT__', $state.endpoint.TrimEnd('/'))
    $body = $body.Replace('__RID__', $state.request_id)
    $body = $body.Replace('__PINFP__', $state.ca_fingerprint)
    $body = $body.Replace('__NEBULACERT__', (Join-Path $BinDir 'nebula-cert.exe'))
    $body = $body.Replace('__NEBULA__', (Join-Path $BinDir 'nebula.exe'))
    $body = $body.Replace('__CFG__', (Join-Path $Root 'config.yml'))
    $body = $body.Replace('__TASK__', $task)
    Set-Content -Path $fin -Value $body -Encoding utf8
    # SYSTEM executes this file: only SYSTEM + Administrators may write it.
    $aclObj = Get-Acl $fin
    ...
    foreach ($sid in 'S-1-5-18', 'S-1-5-32-544') {
        ...
        $aclObj.AddAccessRule((New-Object Security.AccessControl.FileSystemAccessRule($id, 'FullControl', 'Allow')))
    }
    Set-Acl -Path $fin -AclObject $aclObj
    $tr = 'powershell.exe -NoProfile -ExecutionPolicy Bypass -File "' + $fin + '"'
    & schtasks /Create /F /TN $task /SC MINUTE /MO 5 /RU SYSTEM /TR $tr | Out-Null
    if ($LASTEXITCODE -ne 0) { Die 'ACT-09' "could not register the background finisher task (schtasks exited $LASTEXITCODE)." }
    & schtasks /Run /TN $task | Out-Null
    Good "Done -- nothing else to run. This machine now checks every 5 minutes and will"
    Say "fetch its certificate, verify it against the pinned CA, and join the network"
    Say "automatically the moment your request is approved. It stops checking once joined."
    Say "Progress log: $Root\finish.log"
}
...
function Invoke-Activation {
    ...
    # A0 -- the cert pair must have arrived... OR this machine submitted its own request, in
    # which case not-arrived-yet is simply the moment to arm the background finisher (one-run
    # contract, 2026-09-01): the machine polls from here, never the human. Only a hand-relayed
    # install (no saved submission) still gets the save-the-two-files instruction.
    if (-not ((Test-Path $ca) -and (Test-Path $crt))) {
        $hasState = (Test-Path (Join-Path $Root 'enrollment_submission.json')) -or
                    (Test-Path (Join-Path $Root 'uninvited_submission.json'))
        if ($hasState) {
            if ($DryRun) {
                Say "(dry run) the certificate has not arrived yet -- would arm the background"
                Say "finisher (checks every 5 minutes; fetches, verifies against the pinned CA,"
                Say "starts the service, then removes itself). Re-run without -DryRun."
                return
            }
            $isAdmin = ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()
                       ).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)
            if (-not $isAdmin) {
                Say "The certificate has not arrived yet -- that is fine. Run this once in an"
                Say "ELEVATED PowerShell (right-click -> Run as administrator) and this machine"
                Say "will finish joining AUTOMATICALLY the moment your request is approved:"
                Write-Host ""
                Write-Host "  & ([scriptblock]::Create((irm $Base/install.ps1))) -Activate"
                Write-Host ""
                return
            }
            Arm-Finisher -Root $Root -PkiDir $PkiDir -BinDir $BinDir
            return
        }
...
try {
    ...
    # not a gap hidden.
    $PAYLOAD_SHA256 = '56b95d7f3982748da7eab41586a2b9ad8df7d38143d706bf82ce4c35d000e2e7'
                ...
                for ($i = 0; $i -lt 6; $i++) {
                    ...
                    if ($st.state -eq 'queued') {
                        if ($i -eq 0) {
                            ...
                            Show-FinishNextStep
                            Say "(Checking for another minute in case approval is instant -- safe"
                            Say "to close this window any time.)"
                        }
