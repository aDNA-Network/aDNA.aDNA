...
improvement and point at a section of [Content Mapping](/how/publishing/content-mapping) that never described one.
...
This pipeline is the mechanism that publishes the site: **229 pages** as of 2026-09-11, derived from a fresh build, not remembered. The [Convergence Model](/learn/concepts/convergence) describes why vault content is structured for token selection — this pipeline is where that structure pays off on the web side. Every concept, tutorial, and pattern page on adna.network passed through these four stages.
