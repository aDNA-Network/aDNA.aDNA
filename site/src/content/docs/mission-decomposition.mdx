...
description: "A task too large for a single agent session — writing 13 concept files, or building a full documentation site — can't be executed atomically. Without de..."
...
<CardGrid columns={2} cards={[
  { title: "Convergence", href: "/learn/concepts/convergence", description: "The structural principle that makes campaign → mission → objective a convergent context funnel." },
  { title: "Token Selection", href: "/learn/concepts/token-selection", description: "The context-budget discipline that sizes each objective to fit inside a single agent session." },
  { title: "AGENTS.md Routing", href: "/patterns/agents-md", description: "The directory-level routing every objective relies on when claiming its working files." },
  { title: "Run a Campaign", href: "/learn/tutorials/run-a-campaign", description: "Hands-on walkthrough: author a campaign file, decompose it into missions, and log the first session." }
]} />

## Related Elsewhere

<CardGrid columns={2} cards={[
  { title: "Governance Files", href: "/learn/concepts/governance-files", description: "CLAUDE.md, STATE.md, mission files — the governance layer every decomposition writes into." },
  { title: "Knowledge Graph", href: "/learn/concepts/knowledge-graph", description: "Missions connect campaign entities to the files they produce — graph construction, not just planning." },
  { title: "Design a Mission", href: "/learn/tutorials/design-a-mission", description: "Hands-on: author a mission file with objectives, context budget, and handoff notes." },
  { title: "Write a Context File", href: "/learn/tutorials/write-a-context-file", description: "Per-objective context files — the mission-level work that gives each session something to load." }
]} />
