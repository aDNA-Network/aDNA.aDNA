---
title: "aDNA vs. Johnny.Decimal — aDNA Comparisons"
description: "Two systems for organizing files — one uses a universal numbering scheme, the other uses a typed triad with agent governance."
doc_title: "aDNA vs. Johnny.Decimal"
section: "comparisons"
order: 4
---
...
A knowledge architecture standard (§1) that sorts content into three directories (`what/`, `how/`, `who/`) using the [question test](/learn/concepts/triad). Within each leg, typed entity directories (concepts, missions, sessions, etc.) provide further structure. Navigation is governed by AGENTS.md files and typed frontmatter. Designed for AI-agent collaboration on projects.
...
- **Agent routing**: [AGENTS.md](/patterns/agents-md) at each directory tells agents whether to load or skip. Johnny.Decimal numbers mean nothing to an agent.
...
- **No category ceiling**: aDNA can add unlimited entity types via the [base/extension](/patterns/base-extension) pattern. Johnny.Decimal is capped at 100 categories.
...
- [The Triad](/learn/concepts/triad) — aDNA's three-category principle (vs. Johnny.Decimal's numbered areas)
- [Question Test](/patterns/question-test) — how aDNA sorts content (vs. Johnny.Decimal's numbering assignment)
- [Ontology](/learn/concepts/ontology) — the typed entity system that gives aDNA directories semantic meaning
