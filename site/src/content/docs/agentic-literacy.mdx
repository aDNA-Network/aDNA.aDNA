---
title: "Agentic Literacy — aDNA Concepts"
description: "Agentic literacy is the ability to work effectively with AI agents — not just prompting them, but structuring knowledge so agents can find, understand, ..."
doc_title: "Agentic Literacy"
section: "concepts"
order: 11
---
...
**Progressive structure**: Navigate from `README.md` (human entry point) → <GlossaryTooltip term="CLAUDE.md" slug="governance-file" definition="One of the five ALLCAPS governance files at an aDNA project root — the agent's auto-loaded entry point with persona, project map, safety rules, and startup protocol." /> (agent entry point) → `STATE.md` (operational context) → `what/concepts/` (concept library). Each step teaches a literacy concept by requiring you to practice it: following governance files, using the triad, reading <GlossaryTooltip term="AGENTS.md" slug="agents-md" definition="A per-directory guide written for agents — purpose, key files, conventions. A signpost at the entrance to each room in the vault." /> routing.
...
<CardGrid columns={2} cards={[
  { title: "Dual-Audience Writing", href: "/learn/concepts/dual-audience", description: "The writing discipline that makes knowledge serve both humans and agents." },
  { title: "Context Commons", href: "/learn/concepts/context-commons", description: "The vision of shared knowledge resources that agentic literacy makes possible." },
  { title: "Open Standard", href: "/learn/concepts/open-standard", description: "How openness lowers the barrier to developing agentic literacy." },
  { title: "The Triad", href: "/learn/concepts/triad", description: "The organizing principle that is the first lesson in agentic literacy." }
]} />

## Related Elsewhere

<CardGrid columns={2} cards={[
  { title: "Dual-Audience Writing", href: "/patterns/dual-audience", description: "The writing rules that keep knowledge accessible to newcomers without losing developer precision." },
  { title: "AGENTS.md Routing", href: "/patterns/agents-md", description: "Per-directory orientation files that let literacy scale — agents self-route without being told what to load." },
  { title: "Navigate an aDNA Vault", href: "/learn/tutorials/navigate-a-vault", description: "A 15-minute guided tour of a real vault — the reading path that turns conceptual literacy into navigation instinct." },
  { title: "Write a Context File", href: "/learn/tutorials/write-a-context-file", description: "Hands-on: build a context file an agent can actually use — literacy applied at the file level." }
]} />
