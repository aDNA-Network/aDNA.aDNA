...
**Schema**: `what/lattices/lattice_yaml_schema.json` — the JSON Schema that all lattice files validate against. Open it to see the `nodes`, `edges`, `federation`, and `fair` block definitions. A <GlossaryTooltip term="skill" slug="skill" definition="A reusable step-by-step recipe agents follow to accomplish a task. A procedure, not a file blueprint — codified expertise in a repeatable form." /> can also be promoted into the registry as a `lattice_type: skill` — the degenerate lattice form.
...
**The vault itself is a context graph**: The knowledge structure you're navigating — concepts linking to concepts, <GlossaryTooltip term="AGENTS.md" slug="agents-md" definition="A per-directory guide written for agents — purpose, key files, conventions. A signpost at the entrance to each room in the vault." /> routing agents through directories — is a `context_graph` lattice. The nodes are files. The edges are wikilinks and AGENTS.md references. The execution mode is agent-driven graph traversal.

## Next Steps

<CardGrid columns={2} cards={[
  { title: "Ontology", href: "/learn/concepts/ontology", description: "The entity types — module, dataset, lattice — that serve as lattice building blocks." },
  { title: "Knowledge Graph", href: "/learn/concepts/knowledge-graph", description: "The broader connected structure that lattices formalize as declarative YAML." },
  { title: "FAIR Metadata", href: "/learn/concepts/fair-metadata", description: "The metadata envelope that makes lattices findable, shareable, and reusable." },
  { title: "Open Standard", href: "/learn/concepts/open-standard", description: "How federation enables lattice composition across independent aDNA instances." }
]} />

## Related Elsewhere

<CardGrid columns={2} cards={[
  { title: "Federation Readiness", href: "/patterns/federation-readiness", description: "The six checks a lattice must pass before it's safe to compose into another project." },
  { title: "FAIR Envelope", href: "/patterns/fair-envelope", description: "The metadata block that makes a composed lattice discoverable and legally reusable." },
  { title: "Build a Lattice", href: "/learn/tutorials/build-a-lattice", description: "Hands-on: author a validated `.lattice.yaml` — composition's atomic unit." },
  { title: "Federate a Vault", href: "/learn/tutorials/federate-a-vault", description: "Walk through cross-instance composition — external reference vs. inline, with the actual commands." }
]} />

## From the Reference

<CardGrid columns={2} cards={[
  { title: "Specification", href: "/reference/specification", description: "Normative aDNA specification — §9 defines the lattice YAML schema; §11 defines federation readiness checks for composed lattices." },
  { title: "Design Rationale", href: "/reference/design-rationale", description: "Why aDNA chose declarative YAML graphs over imperative scripts — composability and reproducibility as first principles." }
]} />
