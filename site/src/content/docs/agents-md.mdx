...
<CardGrid columns={2} cards={[
  { title: "Knowledge Graph", href: "/learn/concepts/knowledge-graph", description: "The connected structure AGENTS.md is the navigation layer for." },
  { title: "Governance Files", href: "/learn/concepts/governance-files", description: "The five ALLCAPS files at the root — AGENTS.md is the per-directory partner that scales them into every subdirectory." },
  { title: "Navigate an aDNA Vault", href: "/learn/tutorials/navigate-a-vault", description: "Walk a real vault and feel how AGENTS.md guides decisions without being read cover-to-cover." },
  { title: "Write a Context File", href: "/learn/tutorials/write-a-context-file", description: "Hands-on: add a file whose entry the directory's AGENTS.md index now has to describe — routing made concrete." }
]} />
