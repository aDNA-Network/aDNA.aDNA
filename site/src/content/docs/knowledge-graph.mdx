---
title: "The Knowledge Graph — aDNA Concepts"
description: "An aDNA vault is not a filing cabinet — it's a knowledge graph. Files are nodes, wikilinks are edges, and AGENTS.md files are the navigation layer that ..."
doc_title: "The Knowledge Graph"
section: "concepts"
order: 3
---
...
An aDNA vault is not a filing cabinet — it's a knowledge graph. Files are nodes, wikilinks are edges, and <GlossaryTooltip term="AGENTS.md" slug="agents-md" definition="A per-directory guide written for agents — purpose, key files, conventions. A signpost at the entrance to each room in the vault." /> files are the navigation layer that helps agents traverse the graph efficiently. The structure is simultaneously a directory tree (for humans browsing) and a connected graph (for agents reasoning about relationships).
...
An aDNA vault works like the web, not the cabinet. Every file links to related files via wikilinks. A concept file links to the patterns that apply it. A <GlossaryTooltip term="mission" slug="mission" definition="A chunk of work too big for one session. Breaks a goal into objectives, each small enough for a single agent session. Bridges campaigns to sessions." /> links to the context it consumes. A session links to the mission it executes. The result is a navigable graph where you can start anywhere and follow connections to build understanding.
...
```markdown
See [The Triad](/learn/concepts/triad) for the organizing principle.
This pattern applies [the question test](/patterns/question-test).
```
...
<CardGrid columns={2} cards={[
  { title: "The Triad", href: "/learn/concepts/triad", description: "The three-leg structure that organizes the graph's nodes." },
  { title: "Ontology", href: "/learn/concepts/ontology", description: "The entity types that define what kinds of nodes exist." },
  { title: "Convergence Model", href: "/learn/concepts/convergence", description: "How graph traversal narrows context for focused work." }
]} />

## Related Elsewhere

<CardGrid columns={2} cards={[
  { title: "AGENTS.md Routing", href: "/patterns/agents-md", description: "The navigation layer riding on top of the graph — routes agents toward the subgraph they actually need." },
  { title: "The Question Test", href: "/patterns/question-test", description: "The filing rule that keeps every new node placed on the right triad leg — graph hygiene." },
  { title: "Navigate an aDNA Vault", href: "/learn/tutorials/navigate-a-vault", description: "See the graph from a reader's perspective — the 15-minute tour that makes the web visible." },
  { title: "Build a Lattice", href: "/learn/tutorials/build-a-lattice", description: "Formalize a subgraph into declarative YAML — graph made executable." }
]} />
