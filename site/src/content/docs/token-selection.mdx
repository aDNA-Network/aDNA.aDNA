...
<MermaidDiagram chart={`graph LR
    A["Vault — all files"] -->|"filter by campaign"| B["Campaign scope"]
    B -->|"filter by mission"| C["Mission files"]
    C -->|"filter by objective"| D["Session context"]
    D -->|"fit in window"| E["Active tokens"]`} caption="Token selection narrows the knowledge loaded at each stage" />
...
- [Convergence Model](/learn/concepts/convergence) — the structural principle that makes token selection scale across campaign → mission → objective
- [Governance Files](/learn/concepts/governance-files) — the orientation layer that consumes the first ~5K tokens in every session
- [Knowledge Graph](/learn/concepts/knowledge-graph) — the connected structure that AGENTS.md routing traverses during selection
