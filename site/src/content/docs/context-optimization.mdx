...
<MermaidDiagram chart={`graph LR
    signal["High signal\nContext file"] -->|"loads fast, decides well"| agent["Agent\nContext Window"]
    noise["Low signal\nDump file"] -->|"wastes tokens, slows decisions"| agent
    agent --> output["Decision\nor Artifact"]`} caption="Signal-dense context files lead to better, faster agent decisions" />
