...
A quality-scored context file ready for your vault's <GlossaryTooltip term="context library" slug="context-library" definition="The structured knowledge store in what/context/ — organized by topic with subtopics and token estimates so agents load only what they need." />. By the end, you'll have a file that an AI agent can load to make better domain decisions — with a quality score and token estimate.
...
Add the scores to your <GlossaryTooltip term="frontmatter" slug="frontmatter" definition="The YAML metadata block at the top of every aDNA file, wrapped in --- fences. Records type, dates, editor, tags — the file's ID card." />:
...
If this is a new topic, create a topic directory with an <GlossaryTooltip term="AGENTS.md" slug="agents-md" definition="A per-directory guide written for agents — purpose, key files, conventions. A signpost at the entrance to each room in the vault." />. If adding to an existing topic, update the AGENTS.md subtopic table:
...
<CardGrid columns={2} cards={[
  { title: "Context Optimization", href: "/learn/concepts/context-optimization", description: "The design principles your file will be scored against — signal density, format selection, token budget." },
  { title: "Token Selection", href: "/learn/concepts/token-selection", description: "Why the budget exists in the first place — every context file competes for finite session tokens." },
  { title: "AGENTS.md Routing", href: "/patterns/agents-md", description: "The routing layer your context file will be referenced from — AGENTS.md indexes tell agents it exists." },
  { title: "Dual-Audience Writing", href: "/patterns/dual-audience", description: "The writing rule that keeps your file legible to developers and newcomers on the same day." }
]} />
