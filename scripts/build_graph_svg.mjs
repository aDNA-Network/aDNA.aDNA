...
if (prev === output) {
  console.log(`[build_graph_svg] no change — vaults_graph.svg already current (${sizeKB} KB, ${textCount} <text> labels, viewBox ${vbW}×${vbH}).`);
} else {
  fs.mkdirSync(path.dirname(OUTPUT_SVG), { recursive: true });
  fs.writeFileSync(OUTPUT_SVG, output);
  console.log(`[build_graph_svg] wrote ${OUTPUT_SVG} (${sizeKB} KB, ${textCount} <text> labels, viewBox ${vbW}×${vbH}; ${linked.length} connected + ${unlinked.length} grid).`);
}
if (sizeKB > 500) console.warn(`[build_graph_svg] WARN: output is ${sizeKB} KB (> 500 KB budget).`);
process.exit(0);
